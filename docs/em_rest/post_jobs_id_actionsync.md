---
title: "POST /jobs/{ID}?action=sync"
product: "vbr"
doc_type: "em_rest"
...
Starts a backup copy job having the specified ID.
...
To start a backup copy job, send the POST HTTP request to the /jobs/{ID}?action=sync URL.

HTTP Request

|  |
| --- |
| POST https://<Enterprise-Manager>:9398/api/jobs/{ID}?action=sync |
...
The example below starts a backup copy job having ID 78c3919c-54d7-43fe-b047-485d3566f11f.
