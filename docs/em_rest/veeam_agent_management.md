---
title: "Veeam Agent Management"
product: "vbr"
doc_type: "em_rest"
source_url: "https://helpcenter.veeam.com/docs/vbr/em_rest/veeam_agent_management.html"
last_updated: "5/22/2025"
product_version: "13.0.1.1071"
---


In this article

This section contains resources and methods for managing computers with Veeam Agents installed, protection groups, Veeam Agent backup jobs and backups created by Veeam Agent backup jobs.

In this section

* [/agents](agents.md)
* [/agents/discoveredComputers](agents_discoveredcomputers.md)
* [/agents/discoveredComputers/{ID}](agents_discoveredcomputers_id.md)
* [/agents/protectionGroups](agents_protectiongroups.md)
* [/agents/protectionGroups/{ID}](agents_protectiongroups_id.md)
* [/agents/protectionGroups/{ID}/discoveredComputers](agents_protectiongroups_id_discoveredcomputers.md)
* [/agents/jobs](agents_jobs.md)
* [/agents/jobs/{ID}](agents_jobs_id.md)
* [/agents/jobs/{ID}/includes](agents_jobs_id_includes.md)
* [/agents/jobs/{ID}/includes/{ID}](agents_jobs_id_includes_id.md)
* [/agents/backups](agents_backups.md)
* [/agents/backupFiles](agents_backupfiles.md)
* [/agents/backupSessions](agents_backupsessions.md)
* [/agents/restorePoints](agents_restorepoints.md)
* [/agents/agentRestorePoints](agents_agentrestorepoints.md)
* [/agents/agentRestorePoints/{ID}](agents_agentrestorepoints_id.md)
* [/agents/agentRestorePoints/{ID}/mounts](agents_agentrestorepoints_id_mounts.md)
* [/agents/agentRestorePoints/{ID}/mounts/{ID}](agents_agentrestorepoints_id_mounts_id.md)
* [/agents/agentRestorePoints/{ID}/mounts/{ID}/{filepath}](agents_agentrestorepoints_id_mounts_id_filepath.md)

Page updated 5/22/2025

Page content applies to build 13.0.1.1071
