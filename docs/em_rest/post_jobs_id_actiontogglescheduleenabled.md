---
title: "POST /jobs/{ID}?action=toggleScheduleEnabled"
product: "vbr"
doc_type: "em_rest"
source_url: "https://helpcenter.veeam.com/docs/vbr/em_rest/post_jobs_id_actiontogglescheduleenabled.html"
...
Disables or enables a job having the specified ID. A disabled job remains in the list of created jobs on the backup server but does not run according to specified job scheduling settings. To enable a job, you need to perform the Disable operation once again.
...
To disable or enable a job, send the POST HTTP request to the /jobs/{ID}?action=toggleScheduleEnabled URL.

HTTP Request

|  |
| --- |
| POST https://<Enterprise-Manager>:9398/api/jobs/{ID}?action=toggleScheduleEnabled |
...
The example below disables or enables a job having ID 78c3919c-54d7-43fe-b047-485d3566f11f.

|  |
| --- |
| Request:  POST https://localhost:9398/api/jobs/78c3919c-54d7-43fe-b047-485d3566f11f?action=toggleScheduleEnabled    Request Headers:  X-RestSvcSessionId   NDRjZmJkYmUtNWE5NS00MTU2LTg4NjctOTFmMDY5YjdjMmNj    Response:  202 Accepted    Response Body:  <Task xmlns="http://www.veeam.com/ent/v1.0" Type="Task" Href="https://localhost:9398/api/tasks/task-1"> |
