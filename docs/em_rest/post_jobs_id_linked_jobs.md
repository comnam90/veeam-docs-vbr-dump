---
title: "post_jobs_id_linked_jobs"
source_url: "https://helpcenter.veeam.com/docs/vbr/em_rest/post_jobs_id_linked_jobs.html"
...
Adds a backup job to a backup copy job with the specified ID.
...
To add a backup job to a backup copy job with the specified ID, send the POST HTTP request to the /jobs/{ID}/linkedJobs resource.

HTTP Request

|  |
| --- |
| POST https://<Enterprise-Manager>:9398/api/jobs/{ID}/linkedJobs |
...
In the request body, the client must send the ID of the job to be linked. The body of the request must conform to the [XML Schema Definition](em_web_api_specifications.md) of Veeam Backup Enterprise Manager REST API.

The request body must contain the following element:

| Element | Type | Description | Modifiable | Min/Max Occurrence |
| --- | --- | --- | --- | --- |
| LinkedJobUid | String | ID of a backup job that you want to link to the backup copy job. | Yes | 1/1 |
