---
title: "/cdpPolicies/{ID}/includes/{ID}"
product: "vbr"
doc_type: "em_rest"
source_url: "https://helpcenter.veeam.com/docs/vbr/em_rest/cdppolicies_id_includes_id.html"
last_updated: "9/1/2023"
product_version: "13.0.1.1071"
---


In this article

Represents a VM or a VM container having the specified ID and processed by the CDP policy with the specified ID.

Resource URL

|  |
| --- |
| https://<Enterprise-Manager>:9398/api/cdpPolicies/{ID}/includes/{ID} |

Related Resources

* [/cdpPolicies/{ID}](cdppolicies_id.md)
* [/cdpPolicies/{ID}/includes](cdppolicies_id_includes.md)

Methods

The following methods are supported for the /cdpPolicies/{ID}/includes/{ID} resource:

* [GET /cdpPolicies/{ID}/includes/{ID}](get_cdppolicies_id_includes_id.md)
* [DELETE /cdpPolicies/{ID}/includes/{ID}](delete_cdppolicies_id_includes_id.md)

Resource Representation

The /cdpPolicies/{ID}/includes/{ID} resource has a resource representation of the following type:

|  |
| --- |
| <ObjectInJob xmlns="http://www.veeam.com/ent/v1.0" Type="ObjectInJob" Href="https://localhost:9398/api/cdpPolicies/bc933bb6-0cc5-47b6-a1ec-e062b9e29877/includes/460ab29f-c659-4f50-955d-643a370b3744"> |

Page updated 9/1/2023

Page content applies to build 13.0.1.1071
