---
title: "Veeam Cloud Connect"
product: "vbr"
doc_type: "em_rest"
source_url: "https://helpcenter.veeam.com/docs/vbr/em_rest/veeam_cloud_connect.html"
last_updated: "10/3/2025"
product_version: "13.0.1.1071"
---


In this article

This section contains resources and methods for managing Veeam Cloud Connect infrastructure.

|  |
| --- |
| Note |
| Veeam Cloud Connect is supported only on Windows-based installations of Enterprise Manager. |

In this section

* [/cloud](cloud.md)
* [/cloud/gateways](cloudgateways.md)
* [/cloud/gateways/{ID}](cloudgateways_id.md)
* [/cloud/gatewayPools](cloudgatewaypools.md)
* [/cloud/gatewayPools/{ID}](cloud_gatewaypools_id.md)
* [/cloud/tenants](tenants.md)
* [/cloud/tenants/{ID}](tenants_id.md)
* [/cloud/tenants/{ID}/resources](tenants_id_resources.md)
* [/cloud/tenants/{ID}/resources/{ID}](tenants_id_resources_id.md)
* [/cloud/tenants/{ID}/computeResources](tenants_id_computeresources.md)
* [/cloud/tenants/{ID}/computeResources/{ID}](tenants_id_computeresources_id.md)
* [/cloud/tenants/{ID}/vCloudComputeResources](cloud_tenants_id_vcloudcomputeresources.md)
* [/cloud/tenants/{ID}/vCloudComputeResources/{ID}](cloud_tenants_id_vcloudcomputeresources_id.md)
* [/cloud/tenants/{ID}/subtenants](tenants_id_subtenants.md)
* [/cloud/tenants/{ID}/subtenants/{ID}](tenants_id_subtenants_id.md)
* [/cloud/tenants/{ID}/gatewayPools](tenants_id_gatewaypools.md)
* [/cloud/tenants/{ID}/freelicenseCounters](tenants_id_freelicensecounters.md)
* [/cloud/hardwarePlans](hardwareplans.md)
* [/cloud/hardwarePlans/{ID}](hardwareplans_id.md)
* [/cloud/hardwarePlans/{ID}/tenants](hardwareplans_id_tenants.md)
* [/cloud/publicIpAddresses](publicipaddresses.md)
* [/cloud/publicIpAddresses/{ID}](publicipaddresses_id.md)
* [/cloud/cloudFailoverPlans](cloudfailoverplans.md)
* [/cloud/cloudFailoverPlans/{ID}](cloudfailoverplans_id.md)
* [/cloud/cloudFailoverPlans/{ID}/includes](cloudfailoverplans_id_includes.md)
* [/cloud/cloudFailoverPlans/{ID}/includes/{ID}](cloudfailoverplans_id_includes_id.md)
* [/cloud/replicas](cloudreplicas.md)
* [/cloud/replicas/{ID}](cloudreplicas_id.md)
* [/cloud/replicas/{ID}/vmReplicaPoints](cloudreplicas_id_vmreplicapoints.md)
* [/cloud/vmReplicaPoints](cloudvmreplicapoints.md)
* [/cloud/vmReplicaPoints/{ID}](cloudvmreplicapoints_id.md)
* [/cloud/vlans](vlans.md)
* [/cloud/vlans/{ID}](vlans_id.md)
* [/cloud/failoverSessions](cloudfailoversessions.md)
* [/cloud/failoverSessions/{ID}](cloudfailoversessions_id.md)

Page updated 10/3/2025

Page content applies to build 13.0.1.1071
