---
title: "Continuous Data Protection (CDP)"
product: "vbr"
doc_type: "em_rest"
source_url: "https://helpcenter.veeam.com/docs/vbr/em_rest/cdp.html"
last_updated: "5/22/2025"
product_version: "13.0.1.1071"
---


In this article

This section contains resources and methods for managing CDP replica infrastructure set up on all backup servers connected to Veeam Backup Enterprise Manager.

In this section

* [/cdpPolicies](cdppolicies.md)

* [/cdpPolicies/{ID}](cdppolicies_id.md)

* [/cdpPolicies/{ID}/includes](cdppolicies_id_includes.md)

* [/cdpPolicies/{ID}/includes/{ID}](cdppolicies_id_includes_id.md)

* [/cdpPolicies/{ID}/cdpReplicaSessions](cdppolicies_id_cdpreplicasessions.md)

* [/cdpReplicas](cdpreplicas.md)
* [/cdpReplicas/{ID}](cdpreplicas_id.md)

* [/cdpReplicas/{ID}/vms](cdpreplicas_id_vms.md)
* [/cdpReplicas/{ID}/vms/{ID}](cdpreplicas_id_vms_id.md)

* [/cdpReplicaSessions](cdpreplicasessions.md)

* [/cdpReplicaSessions/{ID}](cdpreplicasessions_id.md)
* [/cdpReplicaSessions/{ID}/cdpReplicaTaskSessions](cdpreplicasessions_id_cdpreplicatasksessions.md)

* [/cdpReplicaTaskSessions](cdpreplicatasksessions.md)
* [/cdpReplicaTaskSessions/{ID}](cdpreplicatasksessions_id.md)

Page updated 5/22/2025

Page content applies to build 13.0.1.1071
