---
title: "replication"
source_url: "https://helpcenter.veeam.com/docs/vbr/em_rest/replication.html"
last_updated: "5/22/2025"
product_version: "13.0.1.1071"
---


In this article

This section contains resources and methods for managing replicas created on backup servers connected to Veeam Backup Enterprise Manager.

In this section

* [/replicas](replicas.md)
* [/replicas/{ID}](replicas_id.md)
* [/vmReplicaPoints](vmreplicapoints.md)
* [/vmReplicaPoints/{ID}](vmreplicapoints_id.md)
* [/vmReplicaPoints/{ID}/mounts](vmreplicapoints_id_mountpoint.md)
* [/vmReplicaPoints/{ID}/mounts/{ID}](vmreplicapoints_id_mountpoints_id.md)
* [/vmReplicaPoints/{ID}/mounts/{ID}/{filepath}](vmreplicapoints_id_mountpoints_id_filepath.md)
* [/vAppReplicaPoints](vappreplicapoints.md)
* [/vAppReplicaPoints/{ID}](vappreplicapoints_id.md)
* [/replicaSessions](replicasessions.md)
* [/replicaSessions/{ID}](replicasessions_id.md)
* [/replicaSessions/{ID}/replicaTaskSessions](replicasessions_id_replicatasksessions.md)
* [/replicaTaskSessions](replicatasksessions.md)
* [/replicaTaskSessions/{ID}](replicatasksessions_id.md)
* [/failoverPlans](failoverplans.md)
* [/failoverPlans/{ID}](failoverplans_id.md)

Page updated 5/22/2025

Page content applies to build 13.0.1.1071
