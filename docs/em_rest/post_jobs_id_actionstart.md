---
title: "POST /jobs/{ID}?action=start"
product: "vbr"
doc_type: "em_rest"
source_url: "https://helpcenter.veeam.com/docs/vbr/em_rest/post_jobs_id_actionstart.html"
...
| --- |
| Note |
| To start a backup copy job, send the [POST /jobs/{ID}?action=sync](post_jobs_id_actionsync.md) request. |
...
To start a job, send the POST HTTP request to the /jobs/{ID}?action=start URL.

HTTP Request

|  |
| --- |
| POST https://<Enterprise-Manager>:9398/api/jobs/{ID}?action=start |
