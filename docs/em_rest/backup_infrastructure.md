---
title: "backup_infrastructure"
source_url: "https://helpcenter.veeam.com/docs/vbr/em_rest/backup_infrastructure.html"
last_updated: "5/22/2025"
product_version: "13.0.1.1071"
---


In this article

This section contains resources and methods for setting up and managing backup infrastructure of Veeam Backup & Replication.

In this section

* [/backupServers](backupservers.md)
* [/backupServers/{ID}](backupservers_id.md)
* [/backupServers/{ID}/credentials](backupservers_id_credentials.md)
* [/backupServers/{ID}/credentials/{ID}](backupservers_id_creds_id.md)
* [/backupServers/{ID}/passwords](backupservers_id_passwords.md)
* [/backupServers/passwords{ID}](backupservers_id_passwords_id.md)

* [/backupServers/{ID}/externalRepositories](backupservers_id_externalrepositories.md)
* [/hierarchyRoots](hierarchyroots.md)

* [/hierarchyRoots/{ID}](hierarchyroots_id.md)

* [/managedServers](managedservers.md)

* [/managedServers/{ID}](managedservers_id.md)

* [/repositories](repositories.md)
* [/repositories/{ID}](repositories_id.md)

* [/externalRepositories](externalrepositories.md)

* [/externalRepositories/{ID}](externalrepositories_id.md)
* [/externalRepositories/{ID}/backups](externalrepositories_id_backups.md)
* [/wanAccelerators](wanaccelerators.md)
* [/wanAccelerators/{ID}](wanaccelerators_id.md)

Page updated 5/22/2025

Page content applies to build 13.0.1.1071
