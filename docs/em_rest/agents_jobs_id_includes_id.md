---
title: "/agents/jobs/{ID}/includes/{ID}"
product: "vbr"
doc_type: "em_rest"
source_url: "https://helpcenter.veeam.com/docs/vbr/em_rest/agents_jobs_id_includes_id.html"
last_updated: "9/1/2023"
product_version: "13.0.1.1071"
---


In this article

Represents a protected computer or a protection group having the specified ID and processed by the Veeam Agent backup job with the specified ID.

Resource URL

|  |
| --- |
| https://<Enterprise-Manager>:9398/api/agents/jobs/{ID}/includes/{ID} |

Related Resources

* [/agents/jobs/{ID}](agents_jobs_id.md)

* [/agents/jobs/{ID}/includes](agents_jobs_id_includes.md)

Methods

The following methods are supported for the /agents/jobs/{ID}/includes/{ID} resource:

[GET /agents/jobs/{ID}/includes/{ID}](get_agents_jobs_id_includes_id.md)

Resource Representation

The /agents/jobs/{ID}/includes/{ID} resource has a resource representation of the following type:

|  |
| --- |
| <?xml version="1.0" encoding="utf-8"?> |

Page updated 9/1/2023

Page content applies to build 13.0.1.1071
