...
The following table describes network ports that must be opened to ensure proper communication of Microsoft SQL Server and backup infrastructure components.

| From | To | Protocol | Port | Notes |
| --- | --- | --- | --- | --- |
| Microsoft SQL Server where Veeam Plug-In is installed | Veeam Backup & Replication server | TCP | 10006, 443 | Default port used for communication with the Veeam Backup & Replication server.  Note that data between Veeam Plug-Ins and backup repositories is transferred directly, bypassing the Veeam Backup & Replication server. |
...
| TCP  UDP | 135, 137 to 139 | SMB/Netbios name resolution for SMB protocol (needed in some cases). For details, see [Used Ports](used_ports.md). |
