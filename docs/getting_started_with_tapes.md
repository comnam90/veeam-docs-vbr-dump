...
1. [Add a tape server](adding_tape_server.md) to the Veeam backup server.
2. Load tapes to the tape device (if not yet loaded) and run the [importing procedure](import_tapes.md) for them.
3. Create one or more [media pool](creating_custom_media_pools.md) or [GFS media pool](creating_gfs_media_pools.md) that will be used as targets for tape jobs.
4. Configure one or more [media vaults](creating_media_vaults.md) to conveniently organize offline tapes.
