...
| --- |
| Important |
| If the selected databases and your backup appliance belong to the same Microsoft Entra tenant, the private endpoints will be automatically approved upon creation, and you will not have to take any additional configuration steps. Otherwise, the endpoints will be created with the Pending status, and you will have either to approve them manually as described in [Microsoft Docs](https://learn.microsoft.com/en-us/azure/postgresql/network/how-to-networking-servers-deployed-public-access-approve-private-endpoint?tabs=portal-approve-private-endpoint-connections) or to enable automatic endpoint approval for the service account specified in the policy settings as described in section [Adding Service Accounts](azure_service_account_endpoint_approval.md). |
