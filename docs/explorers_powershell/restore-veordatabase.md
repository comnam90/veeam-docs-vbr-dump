...
| OracleHomePassword | For restoring Oracle Database 12c or later to a Windows machine.  Specifies Oracle home credentials that the cmdlet will use for starting Oracle Services on the guest OS.  This parameter is required in case you use the following types of Oracle home User:   * Existing Windows user. * New Windows user. | SecureString | False | Named | False |
