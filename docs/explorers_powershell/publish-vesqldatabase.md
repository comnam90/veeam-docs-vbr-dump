...
| DatabaseName | Specifies a new name for the Microsoft SQL Server database. The cmdlet will publish the existing database with this name. | String | False | Named | False |
...
| ToPointInTime | Specifies a point in time within the restore interval of a Microsoft SQL Server database.  The cmdlet will publish the database to the state of the specified point in time.  Note: Make sure to first convert the specified point in time to Universal Coordinated Tim (UTC). | DateTime | False | Named | False |
