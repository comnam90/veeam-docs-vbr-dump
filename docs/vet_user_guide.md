---
title: "Veeam Explorer for Microsoft Teams"
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/vet_user_guide.html"
last_updated: "10/31/2025"
product_version: "13.0.1.1071"
---

# Veeam Explorer for Microsoft Teams


Veeam Explorer for Microsoft Teams allows you to restore and export Microsoft Teams data from backups created by Veeam Backup for Microsoft 365. You can also save data from backups to a local computer and send data from backups by email.

In This Section

* [Planning and Preparation](vet_prerequisites.md)
* [Launching Application and Exploring Backups](vet_launching.md)
* [General Application Settings](vet_general_application_settings.md)
* [Standalone Databases and External Servers](vet_standalone_databases.md)
* [Data Restore](vet_restore.md)
* [Data Export](vet_export.md)
* [Saving Posts](vet_saving_posts.md)
* [Saving Files](vet_saving_files.md)
* [Sending Posts and Files](vet_sending_teams_items.md)

* [Getting Support](vet_support.md)


