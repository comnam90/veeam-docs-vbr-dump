...
If you have imported backup copy files created by a [backup copy job](plugins_sap_maxdb_backup_copy.md) from another repository, you can convert them into regular backup files. When you convert backup copy files to regular backup files, Veeam Plug-In creates a backup job and attaches the converted backup files to it. You can use this backup job to continue the backup chain and use converted backup files as a restore point.
...
|  |
| --- |
| SapMaxDBBackintConfigTool --promote-backup-copy-to-primary  Backup copies available for promotion to primary backup:  1. Backup Copy Job 1\srv001 SAP MaxDB backint backup (Default Backup Repository)  Select backup: 1  Promotion of backup copy to a primary backup will reconfigure the plug-in to use a different repository. Continue? (y/N): y |
