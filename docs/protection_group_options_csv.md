...
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/protection_group_options_csv.html"
...
2. Clear the Install backup agent checkbox. Otherwise, Veeam Backup & Replication will install Veeam Agent on the machine. To learn how to use protection groups to automatically deploy Veeam Agents, see [Veeam Agent Backup](protect_comp.md).
...
| --- |
| Note |
| [For Veeam Plug-In for SAP on Oracle] if you plan to back up databases in the rman\_util mode, you must install 2 plug-ins on the Oracle server:   * Veeam Plug-In for Oracle RMAN * Veeam Plug-In for SAP on Oracle |
...
1. If you want to instruct Veeam Backup & Replication to automatically upgrade Veeam Plug-Ins on discovered computers when a new version of the product appears on the distribution server, in the Deployment section, make sure that the Enable auto-update for installed components check box is selected.
...
![Step 5. Specify Discovery and Deployment Options](images/plugins_protection_group_options_csv.webp)
