...
2. Upload the Veeam Plug-In installation package (VeeamPluginforOracleRMAN-13.0.3.29-1.aix6.1.ppc.rpm) to the AIX server where the target Oracle database is deployed.
...
2. Open the setup archive, in the \VeeamPluginforOracleRMAN-13.0.3.29-1\RMAN\_AIX\ppc64 folder, find the VeeamPluginforOracleRMAN-13.0.3.29-1.aix6.1.ppc.rpm file.
3. Upload the Veeam Plug-In installation package (VeeamPluginforOracleRMAN-13.0.3.29-1.aix6.1.ppc.rpm) to the AIX server where the target Oracle database is deployed.

1. Install the plug-in from package with root privileges. Make sure the root user has privileges to add the PKG file.

|  |
| --- |
| rpm -i VeeamPluginforOracleRMAN-13.0.3.29-1.aix6.1.ppc.rpm |
