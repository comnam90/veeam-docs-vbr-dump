...
1. In the Storage-level corruption guard section, select the Perform backup files health check check box and specify the time schedule for the health check. By default, if the health check is enabled, it is performed monthly at 5:00 AM every last Saturday.
