...
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/uni_cdp_policy_sql_trans_logs.html"
last_updated: "2026"
...
The SQL tab is available for workloads that run Microsoft SQL Server and if you have selected Process transaction logs with this job when configuring application-aware processing.
...
To define custom settings for a workload added as a part of a protection group, you must include the workload in the list as a standalone object. To do this, click Add and choose the necessary workload. Then select the workload in the list and define the necessary settings.
...
* To use the account specified in the [protection group from which the workload is added](uni_cdp_service_install.md), select Use guest credentials.
* To use another account, select the required one from the drop-down list. In this case, you can use SQL Server authentication by selecting the Use SQL Server authentication check box.
...
If you have not set up credentials beforehand, click the Manage accounts link or click the Add button to add credentials.
...
![Microsoft SQL Server Transaction Log Settings](images/uni_cdp_policy_sql_transaction_logs.webp "Configure log handling for Microsoft SQL Server")
