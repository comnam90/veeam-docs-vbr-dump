...
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/backup_job_advanced_hv_hv.html"
last_updated: "2026"
...
1. To allow crash-consistent backups when VSS or Hyper-V Integration Services are unavailable on the target VM, select the Allow crash-consistent backup check box. In this case, Veeam Backup & Replication will create a crash-consistent backup of such a VM.
2. In the Changed block tracking section, specify if CBT must be used for VM backup. By default, Use changed block tracking data option is enabled. CBT dramatically reduces the load on the production environment and the time required to perform incremental backups.
...
![Hyper-V Settings](images/hv_backup_job_settings_hv.webp)
