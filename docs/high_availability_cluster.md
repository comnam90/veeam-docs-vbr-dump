...
The High Availability (HA) cluster is a solution designed to ensure continuous availability and minimize downtime of the backup server by clustering two backup servers. The HA cluster addresses the need for resilience and operational continuity of the Veeam Backup & Replication infrastructure. The HA cluster guarantees continuous operation by replicating your backup server, ensuring uninterrupted service.

You can deploy an HA cluster cluster as a standard HA cluster, where the nodes and the cluster reside in the same subnet, or as a cross-subnet HA cluster, where the nodes can reside in different subnets or geographic locations.
...
You can deploy an HA cluster as a standard HA cluster, where the nodes and the cluster reside in the same subnet, or as a cross-subnet HA cluster, where the nodes can reside in different subnets or geographic locations. For more information, see [Specify the cluster mode](high_availability_mode.md).
