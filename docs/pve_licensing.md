...
Veeam Plug-in for Proxmox VE is licensed by the number of protected Proxmox VE VMs. Each protected VM consumes one Veeam Universal License instance from the license scope. A VM is considered protected if it has a restore point created during the past 31 days.
