...
| --- |
| NOTE |
| Consider the following:   * Customization settings are applied only to Veeam Agent for Microsoft Windows version 13.1 or later. Earlier versions ignore the new settings. * Customization settings are visible only on computers that run a Veeam Agent control panel — that is, computers protected by Veeam Agent backup policies (managed by agent). Computers protected by backup jobs managed by the backup server do not have a local Veeam Agent control panel, so customization settings do not affect them. |
