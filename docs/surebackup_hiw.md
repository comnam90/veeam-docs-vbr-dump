...
![Veeam SureBackup – Automatic Recoverability Verification for VM Backups](images/surebackup_hiw_vsphere.webp)
...
1. [Application group](application_group.md). During recovery verification, the verified machine may need to be started with a group of machines on which it is dependent. The application group enables full functionality of applications running inside the machine and lets you run these applications just like in the production environment.
2. [Virtual lab](virtual_lab.md). The virtual lab is the isolated virtual environment in which the verified machine and machines from the application group are started and tested.
3. [SureBackup job](surebackup_job.md). The SureBackup job is a task to perform recovery verification. You can run the SureBackup job manually or schedule it to run automatically by schedule.
