...
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/sangfor_backup_job_create_assign_vms.html"
last_updated: "2026"
...
To view the list of available VM tags, click the Tags icon on the toolbar at the top right corner of the window. If you add a VM tag to the backup scope, Veeam Backup & Replication will regularly check for new VMs included into the added tag and automatically update the backup job settings to include these VMs in the scope. For a cluster to be displayed in the list, it must be configured in the Sangfor Cloud Platform administration portal and must contain at least one VM.
...
| --- |
| Note |
| Consider the following:   * If you include the same resource into the backup scope multiple times (for example, an individual VM and a tag that contains this VM), Veeam Backup & Replication will process this resource only once.  * If you include a cluster or tag into the backup scope, VMs in this object are processed at random. To ensure that the VMs are processed in a specific order, you must add them as standalone VMs — not as part of a tag or cluster. |

[![Select VMs to Back Up](images/sangfor_backup_job_create_resources.webp)](images/sangfor_backup_job_create_resources.webp "Select VMs to Back Up")
...
5. In the Select Disks window, select the Selected Disks option, click Add and select the disks that you want to back up.
...
| --- |
| Note |
| If you exclude disks, total size of a VM may be displayed inaccurately. |

[![Select Disks](images/sangfor_backup_job_create_disk_excludes.webp)](images/sangfor_backup_job_create_disk_excludes.webp "Select Disks")
