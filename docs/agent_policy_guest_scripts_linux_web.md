...
6. In the Job scripts section, specify paths to the executable files on the backup server. These scripts will run before and after the backup policy session.
...
During the backup policy session, Veeam Backup & Replication uploads the scripts to the /var/lib/veeam/scripts/<backup policy ID>/<script type>/ directory on each Veeam Agent computer included in the backup policy. Veeam Agent executes the scripts from the same directory as the root user.
