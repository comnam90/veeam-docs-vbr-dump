...
1. The failover process triggers the replication job to perform another incremental replication run and copy the portion of last-minute changes to the replica. The replica becomes fully synchronized with the source VM.
2. The VM is failed over to its replica.
3. The VM replica is powered on.
...
![Planned Failover](images/planned_failover.webp)
