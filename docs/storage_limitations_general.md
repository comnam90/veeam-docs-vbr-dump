...
* Fsas ETERNUS EP300
* Fsas ETERNUS HX/AX
* Fsas ETERNUS AF and DX Series (management connections only)
...
* The mount server, whether it is a helper host or a helper appliance, must have access over a network to a VM whose files you restore or direct access to vCenter or ESXi host where the VM resides. If the mount server is connected to a VM whose files you restore through vSphere Web Services, you must use a root account for a target VM, otherwise the restore process will fail.
