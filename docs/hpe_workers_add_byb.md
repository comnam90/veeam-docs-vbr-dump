...
* At least one worker must be deployed in each HPE Morpheus VM Essentials environment. It is recommended that workers are deployed on each host of HPE Morpheus VM Essentials clusters. If no worker is deployed on the host, performance of backup and restore operations will be affected since Veeam Backup & Replication will use a worker deployed on another host and switch from the HotAdd transport mode to the NBD transport mode.
