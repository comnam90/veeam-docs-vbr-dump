...
2. You can select a SQL Server instance residing on the local machine on the Local Servers tab or click the Network Servers tab and select an instance on another server. Veeam Explorer for Microsoft SQL Server display servers from your network where SQL Server is installed.
...
2. In the Specify user account to connect section, specify credentials which will be used to connect to the target SQL Server instance.
