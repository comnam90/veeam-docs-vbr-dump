---
title: "Reporting"
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/agents_reporting.html"
last_updated: "5/8/2024"
product_version: "13.0.1.1071"
---

# Reporting


You can view real-time statistics for rescan jobs, as well as Veeam Agent backup jobs and backup policies configured in Veeam Backup & Replication. You can also generate reports with statistics data for performed rescan job or backup job sessions. You can generate reports manually in the Veeam Backup & Replication console or set up Veeam Backup & Replication to send reports automatically by email.

In This Section

* [Viewing Rescan Job Statistics](agents_reporting_stats.md)
* [Viewing Rescan Job Report](agents_reporting_report.md)
* [Viewing Veeam Agent Backup Job Statistics](agents_reporting_job_stats.md)
* [Viewing Veeam Agent Backup Job Report](agents_reporting_job_report.md)
* [Viewing Backup Policy Statistics](agents_reporting_policy_stats.md)
* [Viewing Backup Policy Report](agents_reporting_policy_report.md)
* [Viewing SQL Server Transaction Log Backup Statistics](agent_reporting_logs_statistics.md)
* [Viewing SQL Server Transaction Log Backup Report](agents_reporting_logs_report.md)
* [Enabling Email Reporting](agents_reporting_email.md)


