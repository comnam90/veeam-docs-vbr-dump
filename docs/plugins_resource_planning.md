...
In addition to the [general system requirements for the backup server](system_requirements_backup_server.md), we recommend adding an extra 0.5 CPU cores and 2 GB RAM to your configuration for every 10 machines protected with Veeam Plug-Ins for Enterprise Applications.

Backup Repository

Ensure that the number of available task slots on the Veeam backup repository matches the number of channels (data streams) that you want to run concurrently. By default, Veeam Plug-In uses one channel per data backup operation. You can configure Veeam Plug-In to use multiple channels if needed. Using multiple channels in the backup process improves overall backup performance because Veeam Plug-In distributes the data equally across available channels. At the same time, using multiple channels increases resources consumption on the server with Veeam Plug-In installed, the network and the Veeam backup repository.
