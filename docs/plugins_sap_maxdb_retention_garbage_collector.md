...
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/plugins_sap_maxdb_retention_garbage_collector.html"
...
Veeam Plug-In for SAP MaxDB has a functionality that automatically deletes backup files which are older than specified number of days. For example, you can use it if a backup repository contains backup files that are no longer in the backup catalog.

1. To enable automatic deletion of backup files, run the following command.

|  |
| --- |
| SapMaxDBBackintConfigTool --set-force-delete |
...
| --- |
| Important |
| A value for the number of days setting must be at least 1 backup generation period longer than the retention period for your SAP MaxDB backups. Otherwise, Veeam Plug-In will delete earliest backups created within the retention period. |
