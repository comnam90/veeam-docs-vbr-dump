---
title: "Backup Copy"
product: "vbr"
doc_type: "powershell"
source_url: "https://helpcenter.veeam.com/docs/vbr/powershell/backupcopy.html"
last_updated: "4/22/2024"
product_version: "13.0.1.1071"
---

# Backup Copy


The following topics contain instructions on creating backup copy jobs and managing them by means of Veeam PowerShell.

In this section

* [Backup Copy Jobs for VMs and Physical Machines](backup_copy.md)
* [Backup Copy Jobs for HPE StoreOnce Repositories](catalyst_backup_copy_job.md)
* [Plug-in Backup Copy Job](plugin_backup_copy_job.md)
* [Backup Copy Job of Amazon EC2 Instances](backup_copy_job.md)
* [Azure Backup Copy Job](azure_backup_copy.md)
* [Veeam Agent Backup Copy Job](veeam_agent_backup_copy_job.md)


