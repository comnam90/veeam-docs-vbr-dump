...
| EventId | Specifies the ID of a encrypted data malware event. The cmdlet will analyze this event. | Guid | True | 0 | True (ByPropertyName, ByValue) |
| FolderPath | Specifies a local absolute path to a folder. The cmdlet will save the logs archive of the encryption analysis to this folder. If the folder does not exist, the cmdlet will create it. | String | True | Named | False |
