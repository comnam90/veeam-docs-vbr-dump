...
|  |
| --- |
| Set-VBRAmazonS3Server [-Account <VBRAmazonAccount>] [-BackupIOControlLevel {Lowest | Low | Medium | High | Highest}] [-BucketName <String>] [-CacheRepository <CBackupRepository>] [-Force] [-MetaMigrationType {CheckExistence | CopyMetaFromCache | DownloadMetaFromArchive}] [-ProxyMode {Automatic | SelectedProxy}] [-RetrievalSettings <VBRUnstructuredBackupColdStorageRetrievalSettings>] [-SelectedProxyServer <VBRNASProxyServer[]>] -Server <VBRAmazonS3Server> [<CommonParameters>] |
...
| MetaMigrationType | Specifies how the cmdlet will migrate metadata. You can use one of the following options:   * CheckExistence — use this option to check that metadata is available on the current cache repository. * CopyMetaFromCache — use this option to migrate metadata from source cache repository to a new cache repository. Note: If metadata in a source cache repository is corrupted, the cmdlet will copy data from the archive repository. * DownloadMetaFromArchive — use this option to migrate metadata from archive repository or from replica metadata in archive repository. | VBRUnstructuredBackupMetaMigrationType | False | Named | False |
...
| Force | Defines that the cmdlet will modify settings of Amazon S3 storage without showing warnings in the PowerShell console. | SwitchParameter | False | Named | False |
