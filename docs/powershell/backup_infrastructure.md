---
title: "Backup Infrastructure"
product: "vbr"
doc_type: "powershell"
source_url: "https://helpcenter.veeam.com/docs/vbr/powershell/backup_infrastructure.html"
last_updated: "6/17/2024"
product_version: "13.0.1.1071"
---

# Backup Infrastructure


The following topics contain instructions on setting up and managing the backup infrastructure by means of Veeam PowerShell.

|  |
| --- |
| Important |
| Veeam PowerShell does not support the use of square brackets in the names of Veeam backup infrastructure components. |

In this section

* [Managing Infrastructure](manage_infrastructure.md)
* [VMware Virtual Units](virtual_units.md)
* [Virtual Networks](virtual_networks.md)
* [Virtualization Servers and Hosts](servers.md)
* [Credentials](creds_cmdlets.md)
* [Cloud Credentials](cloud_credentials.md)
* [Proxies](proxies.md)
* [Backup Repositories](repositories.md)
* [External Repositories](external_repositories.md)
* [Scale-Out Backup Repositories](scale_out_repository.md)
* [NDMP Servers](ndmp_servers.md)
* [Object Storage Repositories](object_storage_repositories.md)
* [Network Traffic Rules](network_traffic_rules.md)
* [Locations](gdpr.md)
* [Logs Export](log_cmdlets.md)
* [Backups](files.md)
* [Restore Points](point_cmdlets.md)
* [Encryption](crypto_cmdlets.md)
* [Server Components Upgrade](vbr_update_components.md)
* [Veeam Backup & Replication Settings](vbr_global_settings.md)
* [Roles and Users](roles_and_users.md)


