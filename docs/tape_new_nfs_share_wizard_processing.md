...
![Step 4. Specify File Share Processing Settings](images/nfs_file_share_wizard_processing2.webp)
...
2. Use the Backup I/O control slider to define how fast all proxies can read data from the source file share. This setting is based on the number of parallel threads that can be used by all the proxies configured for processing the file share. If resources of your NAS device are limited, it is recommended that you select the Lower impact option. If your NAS device is powerful enough, select the Faster backup option.
3. Click Apply to save the configured settings.
