---
title: "Launching Background Retention"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/backup_background_retention_launch.html"
last_updated: "1/13/2025"
product_version: "13.0.1.1071"
---

# Launching Background Retention

In this article

Veeam Backup & Replication launches background retention every 24 hours at 00:30. If you need to free up disk space from outdated backups earlier, you can launch the background retention manually.

|  |
| --- |
| Note |
| Be careful when launching background retention to avoid losing necessary data. For more information, see [Background Retention](background_retention_job.md). |

To launch the background retention outside its automatic schedule, do the following:

1. Open the Home view.
2. In the inventory pane, right-click the Backups node.
3. Select Apply retention now or click Apply Retention on the ribbon.

[![Apply background retention](images/background_retention_apply.webp)](images/background_retention_apply.webp "Apply background retention")

Related Links

* [Background Retention](background_retention_job.md)
* [Disabling Background Retention](backup_background_retention_disable.md)

Page updated 1/13/2025

Page content applies to build 13.0.1.1071
