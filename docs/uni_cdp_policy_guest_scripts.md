...
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/uni_cdp_policy_guest_scripts.html"
last_updated: "2026"
...
You can instruct Veeam Backup & Replication to run custom scripts before the CDP policy starts the creation of a long-term restore point and after the policy finishes the creation. For example, these can be pre-freeze and post-thaw scripts for a workload that does not support VSS. The scripts will quiesce the workload file system and application data to bring the workload to a consistent state before the creation of a restore point, and bring the workload and applications to their initial state after the creation finishes.
...
![Script Settings](images/uni_cdp_policy_guest_script.webp "Configure script processing")
