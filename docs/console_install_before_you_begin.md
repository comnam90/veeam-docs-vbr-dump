...
* A machine on which you plan to install the Veeam Backup & Replication console must meet the system requirements. For more information, see [System Requirements](system_requirements_console.md).
...
* We do not recommend installing the Veeam Backup & Replication console on the machine that is used in the role of a different backup infrastructure component, for example, as a backup repository. Different components have different services installed on the server. Upgrading the console creates a version mismatch with other components on that machine, which may cause them to stop working.
