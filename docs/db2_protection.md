---
title: "Performing Backup"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/db2_protection.html"
last_updated: "9/1/2025"
product_version: "13.0.1.1071"
---

# Performing Backup

In this article

After you [configure Veeam Plug-In settings](db2_configure.md), you can use Veeam Plug-In to back up IBM Db2 databases.

Veeam Plug-In backs up IBM Db2 databases according to backup settings that you specify. You can specify what databases to back up, the type of database backups you want to create, and processing settings for backed-up data.

In This Section

* [Considerations and Limitations](db2_protection_limitations.md)
* [Preparing Database](db2_protection_prepare.md)
* [Performing Full Backup](db2_protection_full.md)
* [Performing Incremental Backup](db2_protection_incremental.md)
* [Managing Backup Jobs in Veeam Backup & Replication](db2_protection_job_vbr.md)
* [Managing Backups in Veeam Backup & Replication](db2_protection_backup_vbr.md)

Page updated 9/1/2025

Page content applies to build 13.0.1.1071
