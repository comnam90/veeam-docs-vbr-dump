...
| --- |
| Note |
| * For general requirements for permissions that must be provided to the user account to install and work with Veeam Backup & Replication, see [Permissions](required_permissions.md) for Veeam Backup & Replication.  * If you plan to restore SAP HANA databases using Veeam Explorer for SAP HANA, consider the required permissions listed in [Permissions](vehana_permissions.md) for Veeam Explorer for SAP HANA. * The following permissions apply to Veeam Plug-In for SAP HANA operating in the standalone mode. For more information on permissions for Veeam Plug-Ins managed by Veeam Backup & Replication, see [Permissions for Managed Veeam Plug-Ins](plan_and_manage_permissions.md#hana). |
