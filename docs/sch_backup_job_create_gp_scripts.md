...
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/sch_backup_job_create_gp_scripts.html"
...
If you plan to back up VMs running applications that do not support VSS, you can instruct Veeam Backup & Replication to run custom scripts for those VMs. For example, the pre-freeze script may quiesce the VM file system and application data to bring the VM to a consistent state before Veeam Backup & Replication triggers a VM snapshot. After the VM snapshot is created, the post-thaw script brings the VM and applications to their initial state.
...
| --- |
| Tip |
| If you have added a tag or cluster with Microsoft Windows and Linux VMs to the job, you can select to execute both Microsoft Windows and Linux scripts for the VM container. When the job starts, Veeam Backup & Replication will automatically determine what OS type is installed on the VM and use the required scripts to quiesce this VM. |

[![Pre-Freeze and Post-Thaw Scripts](images/sch_backup_job_create_gp_scripts.webp)](images/sch_backup_job_create_gp_scripts.webp)
