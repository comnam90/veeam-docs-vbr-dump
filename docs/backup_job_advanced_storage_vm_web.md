...
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/backup_job_advanced_storage_vm_web.html"
last_updated: "2026"
...
1. To include dirty data blocks in the backup, clear the Exclude deleted files blocks check box. For more information, see [Deleted File Blocks (BitLooker)](dirty_blocks.md).
...
2. From the Storage optimization list, select the block size that will be used to process VMs. For more information on the data block sizes and how they affect performance, see [Storage Optimization](compression_deduplication.md#optimization).
3. To encrypt backup files, set the Enable backup file encryption toggle to On. In the Password field, select the encryption method you want to use — a password that will generate secret keys or an external KMS server that will generate KMS keys. For more information about backup job encryption, see [Encrypting Backup Jobs](encrypting_backup_jobs.md).

If you have not created the password beforehand, click Add to specify a new password. For more information, see [Password Manager](password_manager.md).
...
[![Specify storage settings](images/vm_backup_job_settings_storage_web.webp)](images/vm_backup_job_settings_storage_web.webp "Specify storage settings")
