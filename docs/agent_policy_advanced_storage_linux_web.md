...
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/agent_policy_advanced_storage_linux_web.html"
last_updated: "2026"
...
3. From the Storage optimization drop-down list, select the size of data blocks: 4 MB, 1 MB, 512 KB, or 256 KB. Veeam Agent for Linux will use data blocks of the chosen size to optimize the size of backup files and policy performance.
...
For backup policies targeted at a Veeam backup repository, all data encryption operations are performed in Veeam Backup & Replication. Encryption settings are passed to the Veeam Agent computer only in case this computer is added to a backup policy targeted at a local drive of a protected computer, at an SMB network shared folder, at an object storage repository added in the direct connection mode, or at a cloud repository. Veeam Backup & Replication passes encryption settings when applying the backup policy to a protected computer.
...
* If you enable or disable encryption for an existing Veeam Agent backup, during the next job session Veeam Agent for Linux will create a full backup file.
* Encryption is not retroactive. If you enable encryption for an existing backup policy, Veeam Agent for Linux will encrypt the backup chain starting from the next restore point created with this policy.
