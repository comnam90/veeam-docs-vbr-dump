...
1. To provide a new name and description for the worker or to modify the number of tasks that the worker is able to handle in parallel, follow the instructions provided in section [Adding Workers](pve_workers_add_vm.md) (step 2).
2. To change the network to which the worker is connected or to specify a new IP address for the worker, follow the instructions provided in section [Adding Workers](pve_workers_add_vm.md) (step 3).
