...
For the basic single-host virtual lab, Veeam Backup & Replication creates one virtual network that is mapped to the necessary production network. Veeam Backup & Replication adds a new virtual switch for the virtual lab. The added virtual switch is only used by VMs started in the virtual lab. There is no routing outside the virtual lab to other networks.
