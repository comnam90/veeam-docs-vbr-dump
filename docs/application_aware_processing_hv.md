...
| --- |
| Important |
| If a VM runs an application that does not support Microsoft VSS (there is no VSS writer for this particular type of application, for example, MySQL), Veeam Backup & Replication will not be able to utilize Microsoft VSS and application-aware processing for this VM. To process such VMs, you can use Microsoft Hyper-V guest quiescence with pre-freeze and post-thaw scripts. For more information, see [Microsoft Hyper-V Guest Quiescence](hyperv_tools_quiescence.md) and [Pre-Freeze and Post-Thaw Scripts](pre_post_scripts_hv.md). |
...
3. Microsoft Hyper-V VSS requestor triggers a VM VSS snapshot.
4. Microsoft VSS resumes quiesced I/O activities on the VM guest OS.
5. Veeam Backup & Replication triggers a snapshot for the volume on which the VM is located.
...
![Application-Aware Processing](images/application_aware_processing_hv.webp)
...
For PostgreSQL clusters managed by Patroni, Veeam Backup & Replication detects the cluster topology during application-aware processing and backs up WAL files from the leader node only. If the leader node changes, Veeam Backup & Replication will continue to back up WAL files from the new leader node. This allows you to restore the cluster to any point in time, including one before the leader node changed.
