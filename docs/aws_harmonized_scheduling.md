...
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/aws_harmonized_scheduling.html"
last_updated: "2026"
...
With harmonized scheduling, the backup appliance can keep restore points created according to a daily, weekly or monthly schedule for longer periods of time:
...
For the backup appliance to use the harmonization mechanism, there must be specified at least 2 different schedules: one schedule will control the regular creation of restore points, while another schedule will control the process of storing restore points. In terms of harmonized scheduling, the backup appliance re-uses restore points created according to a more-frequent schedule (daily, weekly or monthly) to achieve the desired retention for less-frequent schedules (weekly, monthly and yearly). Each restore point is marked with a flag of the related schedule type: the (D) flag is used to mark restore points created according to the daily schedule, (W) — weekly, (M) — monthly, and (Y) — yearly. The backup appliance uses these flags to control the retention period for the created restore points. Once a flag of a less-frequent schedule is assigned to a restore point, this restore point can no longer be removed — it is kept for the period defined in the retention settings. When the specified retention period is over, the flag is unassigned from the restore point. If the restore point does not have any other flags assigned, it is removed according to the retention settings of a more-frequent schedule.

|  |
| --- |
| Note |
| Restore points created according to a more-frequent schedule and less-frequent schedules compose a single backup or snapshot chain. This means that regardless of flags assigned to restore points, the backup appliance adds the restore points to the chain as described in sections [Backup Chain](aws_backup_chain.md) and [Snapshot Chain](aws_snapshot_chain.md). |
...
[![Enabling Harmonized Scheduling](images/aws_harmonized_scheduling_daily.webp)](images/aws_harmonized_scheduling_daily.webp "Enabling Harmonized Scheduling")
...
[![Enabling Harmonized Scheduling](images/aws_harmonized_scheduling_weekly.webp)](images/aws_harmonized_scheduling_weekly.webp "Enabling Harmonized Scheduling")
...
Since 7:00 AM, Monday is specified in weekly scheduling settings, the backup appliance will also assign the (W) flag to this restore point. As a result, 2 flags (D,W) will be assigned to the restore point.
...
[![Enabling Harmonized Scheduling](images/aws_retention_snapshots_daily_weekly.webp)](images/aws_retention_snapshots_daily_weekly.webp "Enabling Harmonized Scheduling")
...
[![Enabling Harmonized Scheduling](images/aws_retention_snapshots_daily_removed.webp)](images/aws_retention_snapshots_daily_removed.webp "Enabling Harmonized Scheduling")
...
[![Enabling Harmonized Scheduling](images/aws_retention_snapshots_daily_rp_removed.webp)](images/aws_retention_snapshots_daily_rp_removed.webp "Enabling Harmonized Scheduling")
...
[![Enabling Harmonized Scheduling](images/aws_retention_snapshots_weekly_rp_removed.webp)](images/aws_retention_snapshots_weekly_rp_removed.webp "Enabling Harmonized Scheduling")
