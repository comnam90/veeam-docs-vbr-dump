...
| Remote access PC | Backup server | TCP | 22 | Optional. Required only for troubleshooting Veeam Software Appliances. |
| UDP, TCP | 3389 | Optional. Required only for troubleshooting Windows-based backup server. |
...
| REST client | Backup server | TCP | 443 | — |
| TCP | 9419 | Required for Swagger. Optional for REST API for backward compatibility as REST API listens on port 443 now. |
...
| Remote access PC | Veeam Infrastructure Appliance | TCP | 22 | Optional. Required only for troubleshooting Veeam Infrastructure Appliances. |
...
| TCP | 6160 | — |
| Mount server running vPower NFS Service | Gateway server working with backup repository | TCP | 6162, 2500 to 3300 | Required for Instant Recovery, SureBackup, and Linux file-level recovery.  The port range 2500-3300 is used for failover if port 6162 is unavailable. |
...
* [Application backup repositories](used_ports.md#abr)
...
The following table describes basic network ports that must be opened to ensure proper communication with  [application backup repositories](application_backup_repository.md).

Application Backup Repositories

| From | To | Protocol | Port | Notes |
| Backup server | Application Backup Repository | TCP | 6960 | — |
