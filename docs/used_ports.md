...
| TCP | Dependent on connection method | — |
...
| REST client | Backup server | TCP | 9419 | — |
...
| Helper appliance,  Helper host | ESXi host | TCP | 443 | Required if restore performed over VIX API/vSphere Web Services.  [For VMware vSphere earlier than 6.5] Not required if vCenter connection is used. In VMware vSphere versions 6.5 and later, port 443 is required by vSphere Web Services. |
