...
A backup server is the configuration, administration and management core of the backup infrastructure. The backup server runs the Veeam CDP Coordinator Service. This service coordinates replication and data transfer tasks and controls resource allocation. It is recommended that you place the backup server in the target site or as a separate unit.

For more information on the backup server, see [Backup Server](backup_server.md).

Source Workloads

Source workloads can be of various types, such as physical machines, virtual machines, or cloud instances. However, Veeam Backup & Replication supports specific operating systems. For more information, see [Universal CDP](platform_support_universal_cdp.md) in [Workloads](platform_support.md).
