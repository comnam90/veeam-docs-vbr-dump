...
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/backup_job_vss_sql_vm.html"
...
2. In the VSS Settings section, select Process transaction logs with this job. For more information, see [VSS settings](backup_job_vss_application_vm.md#vss).
...
* Select Backup logs periodically to back up transaction logs with Veeam Backup & Replication. Veeam Backup & Replication will periodically copy transaction logs to the backup repository and store them together with the image-level backup of the Microsoft SQL Server VM. During the backup job session, transaction logs on the VM guest OS will be truncated. For more information, see [Microsoft SQL Server Log Backup](sql_backup.md).
...
![Microsoft SQL Server Transaction Log Settings](images/vm_backup_job_vss_sql.webp)
