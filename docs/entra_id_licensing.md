...
By default, Veeam Backup for Microsoft Entra ID automatically revokes a license instance from protected users if no new restore points have been created during the past 31 days. However, you can manually revoke license instances from protected users as described in section [Revoking License](revoke_servers.md).
...
| --- |
| Important |
| Protection of conditional access policies, intune policies and logs is included into the Veeam Data Platform Advanced and Premium license packages only. For more information on Veeam license packages, see [Veeam Data Platform Feature Comparison](https://www.veeam.com/veeam_data_platform_feature_comparison_ds.pdf). |
...
If you already use Veeam Backup & Replication and you have spare Veeam Universal License instances on your backup server, they can be used to protect Microsoft Entra ID tenants. You can check the number of available license instances in the Veeam Backup & Replication console as described in section [Viewing License Information](license_view.md).

If you have a legacy perpetual per-socket license, you must obtain Veeam Universal License instances and merge them with the existing perpetual socket license as described in section [Merging Licenses](license_merge.md).
