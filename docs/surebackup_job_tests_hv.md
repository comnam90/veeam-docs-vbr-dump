...
* [Select a role that a machine performs](surebackup_job_tests_hv.md#role)
* [Configure VM startup settings](surebackup_job_tests_hv.md#startup)
* [Select tests that must be performed for the VM](surebackup_job_tests_hv.md#test)
* [For Veeam Backup & Replication on Windows] [Specify credentials for running the verification script](surebackup_job_tests_hv.md#creds)

If all machines in the linked job perform the same role, you can specify startup options and test settings for the whole job in bulk. If machines have different roles, you can granularly specify startup options and test settings for each machine in the job.
...
| --- |
| Important |
| If you specify startup options and tests individually for a machine, Veeam Backup & Replication will apply these options and tests only. Options and tests specified at the level of the SureBackup job will be ignored for this machine. |
...
Machine roles are described in XML files stored in the /var/lib/veeam/sbroles folder on Linux and in the %ProgramFiles%\Veeam\Backup and Replication\Backup\SbRoles folder on Windows. To do this, you need to create a new XML file and specify role and test scripts settings in it. For more information, see [Creating XML files with Machine Roles Description](creating_xml_files_hv.md).
...
To verify machines that perform roles other than those specified in the list, you will have to manually configure startup options and specify test scripts to be run against these machines.
...
| --- |
| Note |
| Veeam Backup & Replication performs a heartbeat test for machines that have Hyper-V Integration Services installed. If Hyper-V Integration Services are not installed, the machine will be started but tests will not be performed. |
