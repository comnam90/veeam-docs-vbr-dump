---
title: "Managing Backups"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/managing_backups.html"
last_updated: "10/20/2025"
product_version: "13.0.1.1071"
---

# Managing Backups

In this article

You can perform the following operations with backups:

* [Viewing Backup Properties](view_backup_properties.md)
* [Upgrading Backup Chain Formats](backup_change_type.md)\*
* [Moving Backups](move_backup.md)
* [Copying Backups](copy_backup.md)
* [Exporting Backups](exporting_backups.md)
* [Detaching Backups from Jobs](detach_backup.md)
* [Removing Backups from Configuration](remove_from_backups.md)
* [Deleting Backups from Disk](delete_backup_from_disk.md)
* [Deleting Backups from Object Storage Repositories](deleting_backups_from_object_storage.md)
* [Deleting Backups from Scale-Out Backup Repositories](deleting_backups_from_sobr.md)
* [Removing Missing Restore Points](remove_missing_point.md)
* [Launching Background Retention](backup_background_retention_launch.md)
* [Disabling Background Retention](backup_background_retention_disable.md)

\* Available only for Microsoft Windows-based backup servers.

Page updated 10/20/2025

Page content applies to build 13.0.1.1071
