...
You can select an application group or skip this step. If the application group is not selected, you must link at least one backup or replication job to the SureBackup job at the [Linked Jobs](surebackup_job_joblink_vm.md) step of the wizard. In this case, when the SureBackup job starts, Veeam Backup & Replication will only run machines from the linked job in the virtual lab and verify these machines.
