...
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/xen_backup_job_create_assign_vms.html"
last_updated: "2026"
...
1. Click Add.
2. In the Add Objects window, choose whether you want to back up all VMs in the pool, only specific VMs or groups of VMs with assigned tags:

To view the list of available resource tags, click the VMs and Tags icon on the toolbar at the top right corner of the window. If you add a tag to the backup scope, Veeam Backup & Replication will regularly check for new VMs included into the added tag and automatically update the backup job settings to include these VMs in the scope. For a tag to be displayed in the list, it must be configured in the Xen administration portal and must contain at least one VM.
...
| --- |
| Note |
| Consider the following:   * If you include the same resource into the backup scope multiple times (for example, an individual VM and a tag that contains this VM), Veeam Backup & Replication will process this resource only once. * If you include a tag or pool into the backup scope, VMs in this object are processed at random. To ensure that the VMs are processed in a specific order, you must add them as standalone VMs — not as part of the tag or pool. |

[![Select VMs to Back Up](images/xen_backup_job_create_resources.webp)](images/xen_backup_job_create_resources.webp "Select VMs to Back Up")
...
5. In the Select Disks window, select the Selected Disks option and select the necessary disks.

Disks that you do not select will be excluded from the backup job.

[![Select Disks and Volume Groups](images/xen_backup_job_create_disk_excludes.webp)](images/xen_backup_job_create_disk_excludes.webp "Select Disks and Volume Groups")
