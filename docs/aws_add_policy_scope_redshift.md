...
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/aws_add_policy_scope_redshift.html"
last_updated: "2026"
...
For an IAM role to be displayed in the list of available roles, it must be added to backup appliances with the Amazon Redshift Clusters Backup operation selected for the role as described in section [Adding IAM Roles](aws_iam_roles_specify_permissions.md). If you have not added the necessary IAM role to backup appliances beforehand, you can do it without closing the Add Redshift Clusters Policy wizard. To do that, click Add and complete the Add IAM Role wizard.

* Select the Organization option if you want to back up Redshift clusters within an AWS Organization. Then, use the Organization drop-down list to specify the source organization identity — select an entire organization or a scope of organizational units whose resources backup appliances will back up.

For an AWS Organization or a scope of organizational units to be displayed in the list of available identities, it must be added to backup appliances as described in section [Adding AWS Organizations](aws_organizations_add.md).
...
For an organizational unit or account to be displayed in the list of available items, it must be part of the source organization identity, and must be included in the scope of organizational units added to backup appliances, as described in section [Adding AWS Organizations](aws_organization_add_scope.md) (step 4).
...
[![Creating Redshift Clusters Backup Policy](images/aws_redshift_backup_add_iam_role.webp)](images/aws_redshift_backup_add_iam_role.webp "Creating Redshift Clusters Backup Policy")
