...
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/backup_job_vss_postgresql_hv.html"
last_updated: "2026"
...
2. From the Specify PostgreSQL account with SUPERUSER role attribute drop-down list, select a user account that Veeam Backup & Replication will use to connect to the PostgreSQL instance. The account must have privileges described in section [Permissions](permissions_guest_processing.md#postgresql). If you have not set up credentials beforehand, click the Manage accounts link or click Add on the right to add credentials.
...
Note that if you plan to select the System user without password (peer or SSPI) authentication method at step 3 of this procedure, you can add a user account in the [Credentials Manager](credentials_manager.md) without specifying the password for the account.

1. In The specified user is section, specify how the user will authenticate against the PostgreSQL instance:

* Select Database user with password if the account you specified at step 2 is a PostgreSQL account, and you entered the password for this account in the Credentials Manager.

|  |
| --- |
| Note |
| If you want Veeam Backup & Replication to use the user name map authentication, select Database user with password and leave the password field empty. Consider the following:   * Guest OS credentials specified at the Guest Processing step of the wizard will be used as the System-Username.  * PostgreSQL account specified at step 2 will be used as the PG-Username.   For more information about the user name maps, see [PostgreSQL documentation](https://www.postgresql.org/docs/current/auth-username-maps.html). |

* Select Database user with password file (.pgpass) if the password for the account you specified at step 2 is defined in the .pgpass configuration file on the PostgreSQL VM. The password file must be located in the user's home directory. For more information about the password file, see [PostgreSQL documentation](https://www.postgresql.org/docs/current/libpq-pgpass.html).
* Select System user without password (peer or SSPI) if you want Veeam Backup & Replication to use the peer authentication method for Linux machines or the SSPI authentication method for Microsoft Windows machines. For Linux machines, Veeam Backup & Replication will use the account you specified at step 2 as the OS account and as the PostgreSQL account to connect to PostgreSQL. For more information about the peer authentication method, see [PostgreSQL documentation](https://www.postgresql.org/docs/current/auth-peer.html). For Microsoft Windows machines, Veeam Backup & Replication uses SSPI authentication: it first tries Kerberos and, if Kerberos is not available, falls back to NTLM.
...
* Select Keep only last <N> days of log backups to keep WAL files for a specific number of days. By default, WAL files are kept for 15 days. If you select this option, you must make sure that retention for WAL files is not greater than retention for the image-level backups. For more information, see [Retention for PostgreSQL WAL Files](postgresql_backup_retention_hv.md).

1. In the Temporary location for archived WAL files field, specify a path to the storage location where you want to keep WAL files. Depending on the guest OS of the PostgreSQL machine, specify a Linux or Microsoft Windows path. The specified path must be accessible to the PostgreSQL server and have sufficient free disk space.
...
![PostgreSQL WAL Files Settings](images/hv_backup_job_vss_postgresql.webp)
