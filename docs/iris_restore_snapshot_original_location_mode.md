...
| --- |
| NOTE |
| Veeam Backup & Replication assumes that the file system paths on the target ODB server are identical to the paths recorded in the backup. If the paths have changed since the backup was created, select Restore to a different location and use path mapping to correct the discrepancy. For details, see [Restoring to Different Location](iris_restore_snapshot_different_location.md). |
