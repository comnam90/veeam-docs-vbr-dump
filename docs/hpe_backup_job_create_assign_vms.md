...
1. Click Add.
2. In the Add Objects window, choose whether you want to back up all VMs in the cluster or server, only specific VMs or groups of VMs assigned tags and labels:

To view the list of available resource tags and labels, click the Tag and labels icon on the toolbar at the top right corner of the window. If you add a tag or label to the backup scope, Veeam Backup & Replication will regularly check for new VMs included into the added tag or label and automatically update the backup job settings to include these VMs in the scope. For a tag or label to be displayed in the list, it must be configured in the HPE Morpheus VM Essentials administration portal and must contain at least one VM. For more information on labels, see [HPE Morpheus VM Essentials documentation](https://support.hpe.com/hpesc/public/docDisplay?docId=sd00007027en_us&page=GUID-58D35AD8-1AE8-4EBC-9DB1-AC6B9C48D4B9.html).
