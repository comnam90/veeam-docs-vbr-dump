...
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/xen_restore_entire_vm.html"
last_updated: "2026"
...
In case of a disaster, you can restore an entire Xen VM from a backup. Veeam Backup & Replication allows you to restore one or more VMs at a time, to the original location or to a new location.

To restore machines to Xen, you can use the following backups:
...
1. [Launch the Entire VM Restore wizard](xen_restore_entire_vm_launch.md).
2. [Select a restore point](xen_restore_entire_vm_select_vms.md).
3. [Choose a restore mode](xen_restore_entire_vm_mode.md).
4. [Specify a target pool](xen_restore_entire_vm_target.md).
5. [Select a storage where VM virtual disks will be stored](xen_restore_entire_vm_storage.md).
6. [Specify a name for the restored VM](xen_restore_entire_vm_name.md).
7. [Configure network settings](xen_restore_entire_vm_network.md).
8. [Specify a restore reason](xen_restore_entire_vm_reason.md).
9. [Verify restore settings](xen_restore_entire_vm_summary.md).
