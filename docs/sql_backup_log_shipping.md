...
* Over vSphere Web Services. In this scenario, Veeam Backup & Replication obtains transaction logs from the VM guest OS over vSphere Web Services, bypassing the network. For each Microsoft SQL Server instance one log creation request is created for all DBs (grouped by instance).
