...
To use persistent agent components, select the Use persistent guest agent check box when specifying application-aware processing settings as described in section [Application-Aware Processing](backup_job_vss_application_hv.md) for VM backup jobs. The Veeam Installer Service will install persistent guest agents during the first run of the backup job.
