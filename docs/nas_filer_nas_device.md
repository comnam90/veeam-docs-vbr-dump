...
If the drop-down list does not display the required device, you must add it to the storage infrastructure. To add the NAS device, click Add New and follow the instructions described in the [Adding NetApp Data ONTAP](netapp_add.md), [Adding Lenovo ThinkSystem DM/DG Series](lenovo_add.md), [Adding Dell PowerScale (formerly Isilon)](dell_powerscale_add.md) or [Adding Nutanix Files Storage](nutanix_add.md) sections, depending on the type of the storage system you use.

1. If you must specify user credentials to access the NAS filer, select the Use the following account to access the NAS filer check box. From the Credentials drop-down list, select a credentials record for a user account that has Full Control permissions on the file system.
...
![Step 2. Select NAS Device](images/nas_filer_nas_device.webp)
