...
* Oracle databases
* PostgreSQL databases (only in Veeam Plug-in for Scale Computing HyperCore v3)

|  |
| --- |
| Note |
| Due to technical limitations, Veeam Plug-in for Scale Computing HyperCore v2 produces only crash-consistent (not application-consistent) backups that in some cases cannot be used for application item restore. |
...
2. In the Veeam Explorer application, perform the steps described in [Veeam Explorers User Guide](https://helpcenter.veeam.com/docs/vbr/explorers/explorers_introduction.html?ver=13).

|  |
| --- |
| Tip |
| As an alternative to application item restore, you can also [perform file-level restore](sch_vm_guest_restore.md) to recover standalone databases. |
