...
After you delete the job, the replicas created by this job are displayed under the Replicas node. If you want to remove replicas from the Veeam Backup & Replication console and configuration database but keep them on hosts, follow the instructions provided in [Removing Replicas from Configuration](remove_replica_from_configuration.md). If you want to remove replicas not only from Veeam Backup & Replication, but also from host storage, follow the steps in [Deleting Replicas from Disk](delete_replica_from_disk.md).
