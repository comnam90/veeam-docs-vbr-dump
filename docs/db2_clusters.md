...
* IBM PowerHA or clusters running IBM AIX
* IBM Tivoli System Automation (TSA) and Pacemaker for clusters running Linux
* Windows Server Failover Clustering for clusters running Microsoft Windows

Backup of Cluster

To back up databases that operate as part of a failover cluster, do the following:

1. Install and configure Veeam Plug-In for IBM Db2 on each node of the cluster. For more information, see [Installing Veeam Plug-In for IBM Db2](db2_install.md) and [Configuring Plug-In](db2_configure.md).
2. Perform the backup on the active cluster node. For more information, see [Performing Backup](db2_protection.md).
...
* In case of IBM PowerHA, IBM TSA and Windows Server Failover Clustering, Veeam Plug-In gets the cluster name automatically.
