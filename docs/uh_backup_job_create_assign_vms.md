...
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/uh_backup_job_create_assign_vms.html"
last_updated: "2026"
...
1. Click Add.
2. In the Add Objects window, choose whether you want to back up all VMs in the cluster, only specific VMs or groups of VMs with tags assigned:

[Applies only if supported by the hypervisor] To view the list of available tags, click the Tag icon on the toolbar at the top right corner of the window. If you add a tag to the backup scope, Veeam Backup & Replication will regularly check for new VMs to which the added tags are assigned and automatically update the backup job settings to include these VMs in the scope.
...
| --- |
| Note |
| Consider the following:   * If you include the same resource into the backup scope multiple times (for example, an individual VM and a tag assigned to this VM), Veeam Backup & Replication will process this resource only once.  * If you include a tag into the backup scope, VMs in this object are processed in descending order by size, starting with the largest. To ensure that the VMs are processed in a specific order, you must add them as standalone VMs — not as part of the tag. |

[![Select VMs to Back Up](images/uh_backup_job_create_resources.webp)](images/uh_backup_job_create_resources.webp "Select VMs to Back Up")
...
[![Select Disks and Volume Groups](images/uh_backup_job_create_disk_excludes.webp)](images/uh_backup_job_create_disk_excludes.webp "Select Disks and Volume Groups")
