...
To communicate with Fujitsu, Veeam Backup & Replication uses [Veeam Data Movers](veeam_transport_service.md). They connect to each other, process data and transfer it over LAN or WAN.
...
Create at least one share on each Fujitsu ETERNUS appliance. Enable VDMS (Veeam Data Mover Service) for the created share. Leave default compression and deduplication settings for the share.

|  |
| --- |
| Note |
| When adding credentials for Fujitsu ETERNUS, use only lower case symbols for the user name. |

Configuration on Veeam Backup & Replication Side

On the Veeam Backup & Replication side, do the following:

1. Add credentials for the Fujitsu ETERNUS user account you created in the previous steps. For more information, see [SSH Credentials](credentials_manager_linux.md).
...
| --- |
| Important |
| You must use different containers if you connect to the Fujitsu ETERNUS appliance from different backup servers. Each backup server can use only one container. |

1. Add the Fujitsu ETERNUS appliance as a managed Linux server using the created credentials. For more information, see [Adding Linux Servers](add_linux_server.md).
...
1. Configure a Fujitsu ETERNUS backup repository. For more information, see [Adding Deduplicating Storage Appliances](dsa_repository_add.md).

* Point the repository at the created share on the Fujitsu ETERNUS appliance.
* Set the Limit maximum concurrent tasks to N option to 10 tasks. You can increase or decrease this limit with assistance from Fujitsu ETERNUS Customer Support.
