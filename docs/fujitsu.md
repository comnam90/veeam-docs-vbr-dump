...
You can use Fsas ETERNUS CS800 deduplicating storage appliances as backup repositories.

Fsas ETERNUS CS800 Deduplication

Fsas ETERNUS CS800 appliances use Fsas ETERNUS CS800 data deduplication technology. During backup data transfer, Fsas analyses blocks in a data stream. Instead of copying redundant data blocks Fsas uses reference pointers to existing blocks on the storage device.

Backup Infrastructure

To communicate with Fsas, Veeam Backup & Replication uses [Veeam Data Movers](veeam_transport_service.md). They connect to each other, process data and transfer it over LAN or WAN.

Veeam Backup & Replication uses the following Veeam Data Movers:

* Veeam Data Mover on the Fsas appliance
...
Fsas ETERNUS hosts Veeam Data Mover permanently. Veeam Backup & Replication installs the [persistent Veeam Data Mover](veeam_transport_service.md) during the initial connection to the appliance and then reuses the data mover when needed. This eliminates the need for runtime components and reduces potential security exposure.

![Fujitsu ETERNUS CS800](images/fujitsu_hiw.webp)
...
Configuration on Fsas ETERNUS Side

Create at least one share on each Fsas ETERNUS appliance. Enable VDMS (Veeam Data Mover Service) for the created share. Leave default compression and deduplication settings for the share.

|  |
| --- |
| Note |
| When adding credentials for Fsas ETERNUS, use only lower case symbols for the user name. |

Configuration on Veeam Backup & Replication Side

On the Veeam Backup & Replication side, do the following:

1. Add credentials for the Fsas ETERNUS user account you created in the previous steps. For more information, see [SSH Credentials](credentials_manager_linux.md).
...
![Fujitsu ETERNUS CS800](images/dedup_port_creds.webp)
...
| --- |
| Important |
| You must use different containers if you connect to the Fsas ETERNUS appliance from different backup servers. Each backup server can use only one container. |

1. Add the Fsas ETERNUS appliance as a managed Linux server using the created credentials. For more information, see [Adding Linux Servers](add_linux_server.md).
...
| --- |
| Note |
| When you add Fsas ETERNUS version 5.3.0 or later, component ports are assigned automatically. |

1. In the Data transfer options section, provide a range of ports that will be used as a transmission channel.

![Fujitsu ETERNUS CS800](images/dedup_ports.webp)
...
1. Configure a Fsas ETERNUS backup repository. For more information, see [Adding Deduplicating Storage Appliances](dsa_repository_add.md).

* Point the repository at the created share on the Fsas ETERNUS appliance.
* Set the Limit maximum concurrent tasks to N option to 10 tasks. You can increase or decrease this limit with assistance from Fsas ETERNUS Customer Support.
