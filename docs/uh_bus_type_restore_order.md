...
When performing [restore of an entire VM](uh_restore_entire_vm.md) that originally resided on a platform other than oVirt KVM or universal hypervisors, Veeam Plug-in for Universal Hypervisor API attaches disks with the restored data to the target VM in a specific order (SATA, VIRTIO SCSI, VIRTIO) by default, taking into account the original disk bus types unless the following limits are exceeded: 5 SATA, 256 VIRTIO SCSI, 256 VIRTIO disks.
...
3. In the Configuration Files section, select the /etc/veeam/platform-service-vrg/appsettings.json or /etc/veeam/platform-service-pl9/appsettings.json file and click Export.
...
2. Back to the web browser, select the /etc/veeam/platform-service-vrg/appsettings.json or /etc/veeam/platform-service-pl9/appsettings.json file, click Import, choose the updated appsettings.json file and click Open.
3. Restart the Veeam Platform9 Platform and Veeam VergeOS Platform services. To do that, navigate to Logs and Services > Services, select veeam-platform-service-pl9.service or veeam-platform-service-vrg.service and click Restart.
...
1. Save the appsettings.json file.
2. Restart the Veeam Platform9 and Veeam VergeOS services.
