...
2. In the Retention Policy field, specify the number of days. Veeam Backup & Replication will keep all restore points created during the specified number of days.
...
| --- |
| Note |
| Backup copy job with restore point-based retention policy does not support partial vApp active full backups. For more information, see [Performing Partial Active Full Backup](vcloud_manage_backup.md). |
