...
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/agent_job_unix_advanced_scripts.html"
last_updated: "2026"
...
1. In the Advanced Settings window, select the Scripts tab.
2. If you want to execute custom scripts before and after the backup job, select the Before the job and After the job check boxes and click Browse to choose executable files from a folder on the backup server:
...
| --- |
| TIP |
| Consider the following:   * Custom scripts that you define in the advanced job settings relate to the backup job itself, not the OS quiescence process on protected computers. To add pre-freeze and post-thaw scripts for Veeam Agent computer OS quiescence, use the [Guest Processing](agent_job_unix_guest_scripts.md) step of the wizard. * You can also specify what scripts will be executed on a Veeam Agent computer before and after the backup job session. To learn more, see [Backup Job and Snapshot Scripts](agent_job_unix_guest_scripts.md). |

![Script Settings](images/agent_job_settings_scripts_unix.webp)
