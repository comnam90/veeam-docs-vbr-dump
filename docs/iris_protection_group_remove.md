...
* Click Remove Group on the ribbon.
* Right-click the protection group and select Remove.

1. If you want to remove backup components from the protected servers, in the displayed window, select the Uninstall Everything check box. With this option selected, Veeam Backup & Replication will remove the protection group from the configuration database and, in addition, uninstall the InterSystems IRIS plug-in, the Veeam Transport Service, and the Veeam Deployer Service from every server in the removed protection group.
