...
1. Use the Veeam Backup & Replication console to log in to Veeam Backup & Replication.
...
1. On the SAP MaxDB server, change the target backup repository in the Veeam Plug-In settings:

|  |
| --- |
| SapMaxDBBackintConfigTool --set-repositories  Available backup repositories:  SID SH2 has been configured |

1. Run the --map-backup command:
