...
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/gfs_retention_policy.html"
...
To mark a backup file for long-term retention, Veeam Backup & Replication can assign to the file the following types of GFS flags: weekly (W), monthly (M) and yearly (Y). The types of GFS flags that Veeam Backup & Replication assigns depend on the configured [GFS retention policy settings](backup_job_gfs_vm.md). Depending on which flag is assigned to a backup, it will be stored for a specified number of weeks, months or years.

|  |
| --- |
| Important |
| Consider the following:   * GFS flags can be assigned only to [full backup files](backup_files.md) created during the time period specified in GFS policy settings. * For backups located in object storage repositories, GFS flags are assigned to active full backup files that match the established GFS policy. If you enable the GFS retention policy but do not schedule active full backups, or the active full backup does not match the GFS policy, Veeam Backup & Replication will generate a synthetic full backup and assign a GFS flag to it. |

As soon as Veeam Backup & Replication assigns a GFS flag to a full backup file, this backup file can no longer be deleted or modified. Also, Veeam Backup & Replication does not apply [short-term retention policy](retention_policy.md) settings to the full backup file — that is, Veeam Backup & Replication ignores the backup file when determining whether the number of allowed backup files is exceeded. For more information on how GFS flags are assigned, see [Assignment of GFS Flags](gfs_how_flags_assigned.md).
...
* GFS retention policy does not apply to [reverse incremental backup chains](reversed_incremental_backup.md).
...
* [Retention policy for deleted items](retention_deleted_vms.md) does not apply to full backup files to which GFS flags are assigned.

Related Topics

* [Assignment of GFS Flags](gfs_how_flags_assigned.md)
* [Removal of GFS Flags](gfs_flags_removal.md)
