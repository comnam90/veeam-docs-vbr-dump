---
title: "Removing Snapshot from Backup Policy"
product: "vbr"
doc_type: "userguide"
...
# Removing Snapshot from Backup Policy


If you want to detach InterSystems IRIS instance storage snapshots from the application backup policy without deleting them from the storage system, you can use the Remove from Job operation. When you remove a snapshot from the job, the snapshots remain on the storage system and continue to be visible in the Veeam Backup & Replication console under Backups > Snapshots (Orphaned). The next run of the application backup policy creates a new set of storage snapshots.
