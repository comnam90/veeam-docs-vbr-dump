...
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/ahv_workers_add_network_web.html"
last_updated: "2026"
...
For best performance, choose the network that is used for Nutanix Controller VM (CVM). For a network to be displayed in the list of the available networks, it must be configured in the Nutanix AHV cluster as described in [Nutanix documentation](https://portal.nutanix.com/page/documents/details?targetId=Web-Console-Guide-Prism-v6_5:wc-network-management-wc-c.html).

1. If DHCP is enabled in the selected network, the IP address of the worker can be obtained automatically.
...
| --- |
| Tip |
| If you want to use a specific network to transfer backed-up data from and to backup repositories, you can configure multiple network interfaces for the worker VM and connect them to different networks. Since workers deployed by Veeam Plug-in for Nutanix AHV are Linux-based VMs, they have the same limitations that apply to machines running the Rocky Linux operating system:   * The network priority must be considered while configuring multiple networks. You can specify the network order using the Up and Down buttons to follow the recommendations described in section [Configuring Multiple Networks](ahv_multiple_networks.md). * DNS settings cannot be configured separately for each network added to the worker. |
...
[![Launch Add Worker Wizard](images/ahv_workers_add_network_web2.webp)](images/ahv_workers_add_network_web2.webp "Launch Add Worker Wizard")
