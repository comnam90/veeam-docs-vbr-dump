...
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/aws_snapshot_manual.html"
last_updated: "2026"
...
Veeam Plug-in for AWS allows you to manually create snapshots of EC2 instances. You can instruct the backup appliance to store the created snapshots in the same AWS Regions where the processed EC2 instances reside, or in a different AWS Region or AWS account.

|  |
| --- |
| Note |
| The backup appliance does not include snapshots created manually in the snapshot chain and does not apply the configured retention policy settings to these snapshots. |
...
For an EC2 instance to be displayed in the list of available instances, an AWS Region where the instance resides must be added to any of [configured EC2 backup policies](aws_add_policy_source_settings.md), and the IAM role specified in the backup policy settings or in the organization settings must have permissions to access the instance. For more information on the required permissions, see [EC2 Backup IAM Role Permissions](aws_role_permissions_backup_ec2.md).

1. Complete the Take Manual Snapshot wizard:

1. At the Account step of the wizard, specify an IAM role whose permissions the backup appliance will use to create the snapshot. The specified IAM role must belong to the same AWS account in which the processed EC2 instances reside.
...
1. At the Settings step of the wizard, you can instruct the backup appliance to keep the created snapshot for a specific number of days. If a snapshot is older than the specified time limit, the backup appliance removes the snapshot from AWS automatically.

|  |
| --- |
| Tip |
| If required, you can remove the created snapshot manually as described in section [Managing Backed-Up EC2 Instance Data](aws_backups_view_ec2.md). |
...
If you choose to copy tags from source volumes, the backup appliance will first create a snapshot of the EC2 instance and assign to the created snapshot AWS tags with Veeam metadata, then the backup appliance will copy tags from the volumes of the processed instance and, finally, assign the copied AWS tags to the snapshot.
...
If you choose to add custom tags to created snapshots, the backup appliance will assign the specified tags right after it creates a snapshot.
...
[![Creating EC2 Snapshots Manually](images/aws_snapshot_manual_tags.webp)](images/aws_snapshot_manual_tags.webp "Creating EC2 Snapshots Manually")
