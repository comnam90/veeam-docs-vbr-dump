...
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/hpe_backup_job_create_gp_postgresql.html"
last_updated: "2026"
...
* Select the Until the corresponding image-level backup is deleted option if you want to remove WAL log backups and the related image-level backups at the same time, according to the retention policy settings specified at [step 4](hpe_backup_job_create_destination.md) of the wizard.
...
Keep in mind that you must create the folder beforehand manually. Also, make sure that there is enough free space in this folder for the log files and [required permissions](hpe_permissions.md#postgresql) are granted to the user account.
...
Depending on the scope of resources that you have specified at [step 5a](hpe_backup_job_create_gp_applications.md) of the wizard, Veeam Backup & Replication will use the selected account in the following way:
...
![PostgreSQL WAL Files Settings](images/hpe_backup_job_create_gp_postgresql.webp)
