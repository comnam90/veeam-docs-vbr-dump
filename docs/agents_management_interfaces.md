...
You can also perform the following operations on backups: view backup properties and permissions, [grant or revoke computer access to backups](agents_protected_computers_access.md), and delete backups from disk.
...
The following operations are not available in the Veeam Backup & Replication web UI:

* Using [Veeam Cloud Connect repositories](agents_cloud_connect.md) as the target for backup jobs.
* The [Move To](agents_protected_computers_move.md) and [Quick Backup](agents_quick_backup_start.md) operations.
* Viewing history and aggregated statistics for backup policies.
...
* Create [Veeam Recovery Media](recovery_media_create.md).
