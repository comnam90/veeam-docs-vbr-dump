...
| Virtualization Platform | Veeam Plug-in for HPE Morpheus VM Essentials supports HPE Morpheus VM Essentials and HPE Morpheus Enterprise Software versions 8.0.13-2 (and later) and 8.1. The following versions of the HPE Morpheus VM Essentials components are required:   * host agent: 3.1.3 * morpheus-ws-node package:3.3.19-1 * alletra plugin: 1.8.1   Veeam Plug-in for HPE Morpheus VM Essentials requires at least one datastore of the Directory Pool, NFS Pool, GFS2 Pool type configured in each HPE Morpheus VM Essentials cluster. The datastore must be in Active state. |
