---
title: "Licensing"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/db2_plugin_licensing.html"
...
A machine with Veeam Plug-In is assumed protected if it has been processed by a Veeam Plug-In backup job in the last 31 days.
...
A machine protected by both Veeam Plug-In and Veeam Backup & Replication will consume a license only once. For example, you have IBM Db2 server that you back up using Veeam Plug-In. You also back up this server using image-level backup functionality of Veeam Backup & Replication. In this case, only one license will be consumed.
...
| --- |
| Important |
| If you have a clustered database, each node of the cluster will consume one License Unit. The License Units are consumed for all cluster nodes, even if Veeam Plug-In is installed only on one of the nodes. |
