...
1. In the Archived logs section, specify if Veeam Backup & Replication must delete archived logs on the Oracle VM:

* Select Do not delete archived logs if you want Veeam Backup & Replication to preserve archived logs on the VM guest OS. When the VM copy job completes, the non-persistent runtime components or persistent components will not delete transaction logs.
...
* Select Delete logs older than <N> hours or Delete logs over <N> GB if you want Veeam Backup & Replication to delete archived logs that are older than <N> hours or larger than <N> GB. The non-persistent runtime components or persistent components running on the VM guest OS will wait for the VM copy job to complete successfully and then trigger transaction logs deletion using Oracle Call Interface (OCI). If the job does not manage to copy the Oracle VM, the logs will remain untouched on the VM guest OS until the next start of the non-persistent runtime components or persistent components.
