...
You can run a health check for an existing InterSystems IRIS application backup policy. During the health check, Veeam Backup & Replication performs a CRC check for metadata and a hash check for data blocks in backup files to verify their integrity. The health check confirms that the restore point is consistent and that data can be restored from it. For details about how the health check works, see [Performing Health Check and Repair for Unstructured Data Backups](unstructured_data_backup_health_check.md).
