...
1. Click Advanced job settings to open the Advanced Settings window.
...
1. To decrease the size of the backup, on the Storage tab, from the Compression level drop-down list, select a compression level for the backup: None, Dedupe-friendly, Optimal, High or Extreme. For more information on data compression, see [Compression and Deduplication](compression_deduplication.md).
...
3. To instruct Scale Computing HyperCore to freeze applications running on VMs while snapshots are taken, switch to the Scale Computing HyperCore tab and select the Enable Scale Computing HyperCore Guest Tools quiescence check box.
4. To configure settings for automated email delivery of job results, on the Notifications tab, select the Send email notifications to the following recipients check box and specify email addresses of recipients. For more information on mail server settings required for email notifications, see [Configuring Email Settings](sch_email_settings.md).
