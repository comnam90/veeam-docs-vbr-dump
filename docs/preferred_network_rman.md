...
1. Access the Veeam Plug-In for Oracle RMAN directory on your Oracle server. The path to the directory differs depending on the OS of the machine where Veeam Plug-In is installed:
...
|  |
| --- |
| OracleRMANConfigTool.exe --add-preferred-network <network\_IP\_address> <position\_in\_list> |
...
The following example shows what the output of the OracleRMANConfigTool command with the --show-preferred-networks parameter can look like on a machine running Microsoft Windows OS:

|  |
| --- |
| OracleRMANConfigTool.exe --show-preferred-networks  Preferred networks:  1. 172.24.29.156  2. 172.24.26.189 |
...
|  |
| --- |
| OracleRMANConfigTool.exe --remove-preferred-network <network\_IP\_address> |
