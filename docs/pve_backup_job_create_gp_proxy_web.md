...
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/pve_backup_job_create_gp_proxy_web.html"
last_updated: "2026"
...
By default, Veeam Backup & Replication automatically chooses which guest interaction proxy to use for each of the processed VMs based on network settings and rules listed in section [Guest Interaction Proxies](guest_interaction_proxy.md). You can also manually limit the list of servers that may be used as proxies — to do that, set the Automatic Selection toggle in the Guest Interaction Proxy window to Off, then select check boxes next to the necessary servers.
...
| --- |
| Important |
| Due to technical limitations, Linux-based proxies cannot access Windows guest OSes in the current version. That is why if you have added Windows-based VMs to the backup scope at [step 3](pve_backup_job_create_assign_vms_web.md) of the wizard, you must also add at least one Microsoft Windows server to the backup infrastructure. |

[![Launch Add Job Wizard](images/pve_backup_job_create_gp_proxy_web.webp)](images/pve_backup_job_create_gp_proxy_web.webp "Launch Add Job Wizard")
