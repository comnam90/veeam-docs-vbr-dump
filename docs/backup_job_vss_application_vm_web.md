...
Before you configure general application-aware processing settings, check that application-aware processing is enabled:
...
* If you want Veeam Backup & Replication to stop the backup process when an error occurs during application-aware processing, select the Require successful processing option.

* If you want to continue the backup process when an error occurs during application-aware processing, select the Try application processing, but ignore failures option. This option guarantees the completion of the backup job. In this case, Veeam Backup & Replication will create a crash-consistent backup instead of a transactionally consistent backup.
...
* If you do not want to enable application-aware processing for the VM, select the Disable application processing option.
...
* [For Microsoft Exchange and Microsoft SQL VMs] If you want Veeam Backup & Replication to process application logs, select the Process transaction logs with this job option and specify settings on the SQL tab. For more information, see [Microsoft SQL Server Transaction Log Settings](backup_job_vss_sql_vm.md).
...
* [For Microsoft Exchange and Microsoft SQL VMs] If you use a third-party backup tool to perform VM guest level backup, and this tool maintains consistency of the database state, select the Only copy transaction logs option. Veeam Backup & Replication will create a copy-only backup for the selected VM. The copy-only backup preserves the chain of full or differential backup files and transaction logs on the VM. For more information, see [Microsoft Docs](http://msdn.microsoft.com/en-us/library/ms191495.aspx).
...
[![Enable application-aware processing](images/vm_backup_job_vss_web.webp)](images/vm_backup_job_vss_web.webp "Enable application-aware processing")
