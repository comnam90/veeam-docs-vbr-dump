...
* WAL files storage — the /var/lib/veeamdata/pgsql/pg\_wal directory on the primary node, where Veeam Backup & Replication stores the delta as WAL files in case the secondary node is not available. Once the connection is established, Veeam Backup & Replication sends this delta to the secondary node.
