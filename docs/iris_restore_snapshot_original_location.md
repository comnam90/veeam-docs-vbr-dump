...
When you restore InterSystems IRIS instance data to the original location, Veeam Backup & Replication writes the data back to the same ODB server and the same file system paths from which the snapshot was created. No path mapping or additional credentials are required. Veeam Backup & Replication uses the connection established during protection group setup.

|  |
| --- |
| Important |
| Before starting the snapshot InterSystems IRIS instance restore, ensure the following conditions are met:   * The InterSystems IRIS instance on the ODB server is stopped. * The restored volumes are unmounted from the target server and no longer in use.   If the instance is still running when Veeam Backup & Replication attempts to overwrite all files located in InterSystems IRIS data paths, the restore will be successful, however the instance will go to an inconsistent state. |
