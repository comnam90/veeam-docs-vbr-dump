...
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/uni_cdp_policy_oracle_trans_logs.html"
last_updated: "2026"
...
The Oracle tab applies to workloads that run Oracle.
...
2. From the Specify Oracle account with SYSDBA privileges drop-down list, select a user account that Veeam Backup & Replication will use to connect to the Oracle databases. The account that you plan to use must have privileges described in section [Permissions](permissions_guest_processing.md#veo).

You can select Use guest credentials from the list of user accounts. In this case, Veeam Backup & Replication will use the account specified in the [protection group from which the workload is added](uni_cdp_service_install.md).
...
* If you want to preserve archived logs on the guest OS, select Do not delete archived logs. When the replication job completes, the non-persistent runtime components or persistent components will not truncate transaction logs.
...
![Oracle Archived Log Settings](images/uni_cdp_policy_oracle_transaction_logs.webp "Configure log handling for Oracle Server")
