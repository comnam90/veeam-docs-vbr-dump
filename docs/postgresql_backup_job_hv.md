...
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/postgresql_backup_job_hv.html"
...
To back up WAL files, you must enable application-aware processing for PostgreSQL VMs. After this, during the job session Veeam Backup & Replication installs non-persistent runtime components or uses (if necessary, installs) persistent agent components on this VM to collect information about the instance and process WAL files according to job settings. For more information on how to configure application-specific settings for PostgreSQL WAL files backup, see [PostgreSQL WAL Files Settings](backup_job_vss_postgresql_hv.md) step of the backup job wizard — you can specify how logs should be backed up and deleted for PostgreSQL instances.
...
1. The non-persistent runtime components or persistent agent components copy WALs from the log archive destination (set in the [guest processing settings](backup_job_vss_postgresql_hv.md) of a job) to a temporary directory on the VM guest file system.
