...
* Veeam Deployer Service installs and manages the Veeam Transport Service and the InterSystems IRIS plug-in on the ODB server, and is used to establish the initial certificate-based connection.
...
1. Veeam Backup & Replication reads the list of InterSystems IRIS instances and their storage volumes collected during the protection group rescan and stored in the configuration database. Each policy run performs its own rescan to check this topology against the configuration database.
