...
4. To protect the replica from corruption after permanent failover is complete, Veeam Backup & Replication reconfigures the current universal CDP policy by adding the source workload to the list of exclusions. Note that other policies and jobs are not modified automatically. When the CDP policy starts, the source workload is skipped from processing. As a result, no data is written to the working VM replica.
5. Veeam Backup & Replication powers on the replica.
