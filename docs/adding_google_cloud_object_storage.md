---
title: "Adding Google Cloud Object Storage"
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/adding_google_cloud_object_storage.html"
last_updated: "2026"
product_version: "13.1.0.411"
---

# Adding Google Cloud Object Storage


For more information about Google Cloud object storage, see [this Google article](https://cloud.google.com/storage/docs/introduction).

Before you add a Google Cloud object storage to the backup infrastructure, check [prerequisites](google_limitations.md) and [required permissions](permissions_object_storage.md#gcos). If you plan to use immutability, verify that you have configured the [necessary settings](immutability_os_enable.md#google) for your Google Cloud bucket. After that, use the New Object Repository wizard.

1. [Launch the New Object Storage Repository wizard](google_cloud_new_object_repository.md).
2. [Specify the object storage name](google_cloud_repository_name.md).
3. [Specify object storage account](google_cloud_repository_account.md).
4. [Specify object storage settings](google_cloud_storage_details.md).
5. [Specify mount server settings](google_cloud_storage_mount_server.md).
6. [Review components](google_cloud_storage_review.md).
7. [Apply settings](google_cloud_storage_review.md).
8. [Finish working with the wizard](google_cloud_finishing_wizard.md).

Page updated 2026-07-21

