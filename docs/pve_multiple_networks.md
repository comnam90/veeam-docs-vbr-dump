...
In this example, the workers, the backup server, the repository and the Proxmox VE host are connected to Network A, while the workers and the backup repository are also connected to Network B that is [configured as a preferred network](select_backup_network.md) to deliver traffic to the backup repository. This configuration is valid since all backup infrastructure components are connected to the same network.
