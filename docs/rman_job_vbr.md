...
* For an Oracle cluster nodes, Veeam Backup & Replication generates the backup job name based on single client access name (SCAN) of the cluster.

|  |
| --- |
| Note |
| The progress bar of a running Oracle database backup job is available only for backups of standalone Oracle databases. It is not available for Oracle databases that operate in clusters. |
