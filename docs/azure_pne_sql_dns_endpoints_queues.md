...
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/azure_pne_sql_dns_endpoints_queues.html"
last_updated: "2025"
...
1. In the Private endpoint connections tab of the Networking window of the Veeam storage account selected at [step 6a](azure_pne_sql_dns_endpoints_locate.md), locate the private endpoint created for Azure Queue Storage. To do that, click the link in the Private endpoint column. The private endpoint for Azure Queue Storage will have the queue value set in the Target sub-resource field.
...
2. From the Private DNS zone drop-down list, select the pair of the privatelink.queue.core.windows.net name and the resource group in which the DNS zone was created. Leave the default settings for the other options in this window.
3. Click Add.

1. [This step applies only if you chose to create and manage the private DNS zones manually at [step 1](azure_pne_sql_dns_zones.md)] In the private DNS zone, create an 'A' record for the added private endpoint as described in [Microsoft Docs](https://learn.microsoft.com/en-us/azure/dns/dns-getstarted-portal#create-a-dns-record).
...
4. In the Add virtual network link window, add to the DNS zone links to the VNet to which the backup appliance is connected, and to VNets to which the worker instances are connected. To do that, perform the following steps for each VNet link:
...
[![Configuring Private Endpoint for Queues](images/azure_private_endpoint_queue_vnet_links.webp)](images/azure_private_endpoint_queue_vnet_links.webp "Configuring Private Endpoint for Queues")
