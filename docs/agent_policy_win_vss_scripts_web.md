...
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/agent_policy_win_vss_scripts_web.html"
last_updated: "2026"
...
During the backup policy session, Veeam Backup & Replication uploads the scripts to the C:\ProgramData\Veeam\ScriptFiles\Upload\<backup policy ID>\<script type>\ folder on each Veeam Agent computer included in the backup policy. On each protected computer, Veeam Agent copies the scripts to the C:\ProgramData\Veeam\ScriptFiles\<backup policy ID>\<script type>\ folder and executes them from that location under the user whose access credentials are specified in the backup policy settings.

[![Specify Pre-Freeze and Post-Thaw Scripts](images/agent_policy_vss_scripts_web.webp)](images/agent_policy_vss_scripts_web.webp "Specify Pre-Freeze and Post-Thaw Scripts")
