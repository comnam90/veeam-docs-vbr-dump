---
title: "Guide for Service Providers"
product: "vbr"
doc_type: "cloud"
source_url: "https://helpcenter.veeam.com/docs/vbr/cloud/cloud_connect_admin_guide.html"
last_updated: "11/4/2025"
product_version: "13.0.1.1071"
---

# Guide for Service Providers


The Guide for Service Providers is intended for SPs who expose cloud repository resources and provide disaster recovery as a service to their customers using the Veeam Cloud Connect functionality in Veeam Backup & Replication. The guide describes main tasks that the SP must take to set up the necessary infrastructure and manage it.

In This Section

* [Setting Up SP Veeam Cloud Connect Infrastructure](cloud_connect_configure.md)
* [Managing Tenant Accounts](cloud_connect_manage.md)
* [Managing Subtenant Accounts on SP Side](cloud_connect_subtenants_manage.md)
* [Restoring Data from Tenant Backups](cc_data_restore.md)
* [Managing Tenant Data](cloud_connect_manage_data.md)
* [Managing Tenant Cloud Failover Plans](cloud_failover_plan_manage_sp.md)
* [Using Remote Access Console](cc_remote.md)
* [Managing SP Backup Server](cc_backup_server_manage.md)
* [Working with Tapes](cc_backup_to_tape.md)
* [Reporting](reporting.md)
* [Support for Veeam Backup for Public Clouds](cc_public_cloud_backups.md)

Related Topics

* [Getting Started with Veeam Cloud Connect Backup](cloud_connect_backup_getting_started.md)
* [Getting Started with Veeam Cloud Connect Replication](cloud_connect_replication_getting_started.md)


