...
Typically, it is recommended that you use Veeam application-aware processing that leverages the Microsoft VSS framework to prepare the VM guest OS for backup. If you cannot use application-aware processing for some reason, set the Enable Hyper-V guest quiescence toggle to On. Veeam Backup & Replication will select the backup method in the following way:
...
3. If you have added several VMs to the job, in the Volume Snapshots section, set the Allow processing of multiple VMs with a single volume snapshot toggle to On. This option helps reduce snapshot workload in the Microsoft Hyper-V environment. When the job starts, Veeam Backup & Replication will group VMs and trigger Microsoft Hyper-V to create one volume snapshot for a group of VMs instead of a volume snapshot per each processed VM.
...
[![Specify Hyper-V settings](images/hv_backup_job_settings_hv_web.webp)](images/hv_backup_job_settings_hv_web.webp "Specify Hyper-V settings")
