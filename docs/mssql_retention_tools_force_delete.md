...
The way you start the force delete operation depends on the Veeam Plug-In operation mode. If Veeam Plug-In operates in the standalone mode, you must start the force delete operation manually. For more information, see [Force Deleting Backups](plugins_mssql_retention_force.md). If Veeam Plug-In is managed by Veeam Backup & Replication and the setting is enabled in the backup policy settings, Veeam Backup & Replication starts the force delete operation automatically during each backup policy run. For more information, see [Storage Settings](policy_microsoft_sql_server_advanced_storage.md).
