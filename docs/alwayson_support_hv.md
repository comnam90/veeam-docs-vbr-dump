...
Veeam Backup & Replication also detects to what cluster the database belongs. Veeam Backup & Replication also detects to which cluster a database belongs. If the backup job does not include all VMs from the cluster, Veeam Backup & Replication displays an informational message. Including all such VMs is not required for backup processing.
...
* The necessary Veeam Backup & Replication components must be installed on this node and the VM included in Always On Availability Group must be running. For more information on the necessary components, see [How Microsoft SQL Server Log Backup Works](sql_backup_hiw_hv.md).
...
* Veeam Backup & Replication can establish a network connection to the node or PowerShell Direct connection if a connection over the network cannot be established. PowerShell Direct is used for VMs that reside on Microsoft Hyper-V Server 2016 (or later) and run Microsoft Windows 10 (or later) or Microsoft Windows Server 2016 (or later). Veeam Backup & Replication requires Microsoft PowerShell 2.0 (or later) to work over PowerShell Direct.
