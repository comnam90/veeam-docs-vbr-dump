...
* Connect using certificate-based authentication. Select this option, if you chose to pre-install Veeam Deployer Service on the Linux computer that you want to add to the backup job. In this case, Veeam Backup & Replication will make the first communication with the computer using a temporary certificate. To learn more, see [Deploying Veeam Agent Using Veeam Deployment Kit](agents_deploy_deployer.md).
