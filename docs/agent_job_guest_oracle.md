...
To learn more, see [Guest Processing Settings](agent_job_guest.md).
...
To learn more, see [Application-Aware Processing](agent_job_guest_general.md).
...
* Select Delete logs older than <N> hours or Delete logs over <N> GB if you want Veeam Agent for Linux to delete archived logs that are older than <N> hours or larger than <N> GB. Veeam Agent for Linux will wait for the backup job to complete successfully and then trigger archived logs truncation through Oracle Call Interface (OCI). If the backup job fails, the logs will remain untouched until the next successful backup job session.
...
![Oracle Processing Settings](images/agent_job_guest_oracle.webp)
