...
1. Click the Guest OS credentials link.
...
To display a user in the Credentials list, you must add the user to the Credentials Manager as described in section [Credentials Manager](credentials_manager.md). If you have not added the necessary user to the Credentials Manager beforehand, you can do it without closing the New Job wizard. To do that, click the Add button, and specify the user name, password and description in the Guest OS Credentials window.
...
To check whether Veeam Backup & Replication can connect to the VM guest OSes using the specified credentials, click Verify network connectivity and credentials in the Guest Processing window. Veeam Backup & Replication will use the specified credentials to connect to all VMs in the list.
