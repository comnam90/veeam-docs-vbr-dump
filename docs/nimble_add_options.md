...
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/nimble_add_options.html"
last_updated: "8/11/2023"
...
[This step is available if you have selected the Block or file storage for VMware vSphere check box at the [Specify Storage Name or Address and Storage Role](nimble_add_name.md) step of the wizard.]
...
1. To rescan storage systems and perform backup from storage snapshots, you must configure a proxy. On the right of the Backup proxies to use field, click Choose and define proxies that you want to use for these operations.
...
1. From the Windows mount server list, select a Microsoft Windows server that you want to use as a mount server for file-level and application items restore from Windows machines. You can click Add New to add a new server. For more information, see the [Mount Server](mount_server.md) section in Veeam Backup & Replication User Guide.
2. From the Linux mount server list, select a Linux server that you want to use as a mount server for file-level and application items restore from Linux machines. You can click Add New to add a new server. For more information, see the [Mount Server](mount_server.md) section in Veeam Backup & Replication User Guide.

![Step 4. Specify VMware Access Options](images/nimble_add_options.webp)
