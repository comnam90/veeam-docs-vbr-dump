...
* During the installation, the veeamadmin account is automatically added to the Veeam Backup & Replication console with the Backup Administrator role.
...
* The user account under which the Veeam Backup Service runs must have the Backup Administrator role. If you change the default settings, make sure that you assign this role to the necessary user account. It is recommended to assign the role to the user account explicitly rather than the group to which the user belongs.
* The Backup Administrator role is assigned to all members of the Administrators group on the machine where Veeam Backup & Replication is installed.
...
* If you delete the built-in Windows Administrators group in the users and roles settings, local and domain members of this group will immediately lose access to Veeam Backup & Replication.
