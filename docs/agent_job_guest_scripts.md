...
During the backup job session, Veeam Backup & Replication uploads the scripts to the /var/lib/veeam/scripts/<backuз job ID>/<script type>/ directory on each Veeam Agent computer included in the backup job. Veeam Agent executes the scripts from the same directory under the root user.
