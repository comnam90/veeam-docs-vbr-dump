...
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/backup_job_vss_oracle_vm.html"
...
2. In the Specify Oracle account with SYSDBA privileges section, specify a user account that Veeam Backup & Replication will use to connect to the Oracle database. The account must have privileges for the Oracle database listed in the section [Performing Guest Processing](required_permissions.md#rptcb).
...
| --- |
| Note |
| [For Windows-based machines] Make sure you add the %ORACLE\_HOME%\bin variable to the environmental path variable on the machine with your Oracle database. |
...
1. To back up Oracle archived logs with Veeam Backup & Replication, select the Backup log every <N> minutes check box and specify the frequency for archived log backup. By default, archived logs are backed up every 15 minutes. The maximum log backup interval is 480 minutes.
...
+ Select Keep only last <N> days of log backups to keep archived logs for a specific number of days. By default, archived logs are kept for 15 days. If you select this option, you must ensure that retention for archived logs is not greater than retention for the image-level backups. For more information, see [Retention for Archived Log Backup](oracle_backup_retention.md).
...
![Oracle Archived Redo Log Settings](images/vm_backup_job_vss_oracle.webp)
