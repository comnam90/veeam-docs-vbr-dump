...
product: "vbr"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vbr/userguide/agent_job_unix_advanced_storage.html"
last_updated: "2026"
...
3. In the Storage optimization section, select what type of backup target you plan to use. Depending on the chosen storage type, Veeam Agent will use data blocks of different size to optimize the size of backup files and job performance: 4 MB, 1 MB, 512 KB or 256 KB.
...
| --- |
| NOTE |
| If you plan to encrypt the content of backup files, consider the limitations listed in the [Data Encryption Considerations and Limitations](#encrypt_limits) subsection. |

![Storage Settings](images/agent_job_settings_storage_unix.webp)
...
* If you enable encryption for an existing Veeam Agent backup, during the next job session Veeam Agent will create a full backup file. The created full backup file and subsequent incremental backup files in the backup chain will be encrypted with the specified password.
* Encryption is not retroactive. If you enable encryption for an existing backup job, Veeam Agent will encrypt the backup chain starting from the next restore point created with this job.
