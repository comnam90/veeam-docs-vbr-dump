...
3. In the working area, select the necessary job or workload. Note that you can copy backups of an individual workload only if its backups are [per-machine backups](per_vm_backup_files_hv.md) with separate metadata files.
